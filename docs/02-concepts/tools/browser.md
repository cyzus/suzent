...
1. In Settings, click **Open Chrome Web Store** or **Open Edge Add-ons** and install
   ...
   in the browser profile you want Suzent to use.
2. Back in Suzent, click **Pair browser**. The default browser opens a local pairing
...
3. Settings automatically shows **Browser extension connected**. Ask the agent to
...
For manual installation in Chrome or Edge, expand **Setup help & options** and use
the local extension folder (`extensions/browser/` in your Suzent checkout), or
click **Download extension** and extract the ZIP into a permanent folder. Open
Extensions → Manage extensions, enable Developer mode, choose **Load unpacked**,
and select the folder containing `manifest.json`.

Browser extension installation and browser permission prompts cannot be skipped.
Enterprise policies may prohibit extension installation or debugger access.
