...
- Text receive/send and image receive are supported. Incoming images are downloaded from WeChat's CDN and decrypted locally before being passed to the agent. Other media receive is preserved as attachment metadata; media sending is not implemented yet.
