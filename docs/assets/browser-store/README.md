...
The upload ZIP is built and released automatically. Bump `version` in
`extensions/browser/manifest.json` in the same PR as any change to packaged
extension code - CI fails the PR otherwise, because the store copy can only be
updated at a new version and an older extension degrades silently rather than
erroring. On merge, `.github/workflows/browser-extension.yml` tags
`browser-v<version>` and publishes a GitHub release with the ZIP attached.

The extension versions independently of the app; `scripts/bump_version.py` does
not touch its manifest, and `browser-v*` tags are excluded from app release
tooling. Downloading the release asset is all that is needed before submitting
to either store.
...
It writes `store-upload/suzent-browser-<version>.zip` - the contents of
`extensions/browser/` with `manifest.json` at the ZIP root, excluding dotfiles
and `.DS_Store`. Do not include this store kit, Python source, or the repository
in the extension upload; PNG icons are already referenced by the manifest. The
build is reproducible, so a local rebuild can be compared byte-for-byte against
the released asset.
