...
const FACES = [
  {
    ...
    ny: 0,
    nz: 1,
    pts: VALS.flatMap((u) => VALS.map((v) => ({ x: u, y: v, z: 1 }))),
  },
  {
    nx: 0,
    ny: 0,
  ...
  },
  {
    ...
    ny: 0,
    nz: 0,
    pts: VALS.flatMap((u) => VALS.map((v) => ({ x: 1, y: u, z: v }))),
  },
  {
    nx: -1,
    ny: 0,
  ...
  },
  {
    ...
    ny: 1,
    nz: 0,
    pts: VALS.flatMap((u) => VALS.map((v) => ({ x: u, y: 1, z: v }))),
  },
  {
    nx: 0,
    ny: -1,
  ...
  CUBE_EYE_STATIC[i] = p.isFront
    ? EYE_CTRS.reduce(
        (best, e) =>
          Math.max(
            best,
            Math.max(0, 1 - Math.hypot(p.x - e.x, p.y - e.y) / EYE_R) ** 2,
          ),
        0,
      )
    : 0;
...
function genRingArcSegments(def: (typeof RING_DEFS)[number]): number[] {
  ...
  for (let i = 0; i < segN; i++) {
    ...
    if (!dashGate || isRingGap(a0, def.gapN) || isRingGap(a1, def.gapN))
      continue;

    pts.push(
      Math.cos(a0) * def.radius,
      0,
      Math.sin(a0) * def.radius,
      Math.cos(a1) * def.radius,
      0,
      Math.sin(a1) * def.radius,
    );
  }
...
function spawnAsh(): Ash {
  ...
  return {
    x: Math.cos(a) * r,
    y: -2.8 - Math.random() * 0.8,
    z: Math.sin(a) * r,
    vx: (Math.random() - 0.5) * 0.003,
    vy: 0.006 + Math.random() * 0.016,
    vz: (Math.random() - 0.5) * 0.003,
    size: 0.5 + Math.random() * 1.6,
    baseOp: 0.05 + Math.random() * 0.2,
    phase: Math.random() * Math.PI * 2,
    wobble: 0.006 + Math.random() * 0.016,
  };
}
  ...
  for (let shell = 0; shell < 9; shell++) {
    ...
    for (let i = 0; i < n; i++) {
      ...
      pts.push({
        ...
        radius:
          radius +
          Math.sin(t * TWO_PI * wave + shell) * 0.18 +
          (Math.random() - 0.5) * 0.08,
        y:
          Math.sin(t * TWO_PI * wave + shell * 0.7) * (0.58 + shell * 0.09) +
          (Math.random() - 0.5) * 0.08,
...
void main() {
  ...
  gl_FragColor = vec4(uColor + glow * 0.5, a);
}`;
...
export function DotCube({ pointer }: DotCubeProps) {
  ...
  useEffect(() => {
    ptrRef.current = pointer ?? { x: 0, y: 0, active: false };
  }, [pointer]);
  ...
  useEffect(() => {
    ...
    cubeGeo.setAttribute(
      "position",
      new THREE.BufferAttribute(CUBE_POS_STATIC.slice(), 3),
    );
    cubeGeo.setAttribute(
      "aNormal",
      new THREE.BufferAttribute(CUBE_NORM_STATIC.slice(), 3),
    );
    cubeGeo.setAttribute(
      "aEye",
      new THREE.BufferAttribute(CUBE_EYE_STATIC.slice(), 1),
    );
    ...
    dustGeo.setAttribute(
      "aOpacity",
      new THREE.BufferAttribute(dustOp, 1).setUsage(THREE.DynamicDrawUsage),
    );
    ...
    for (let ri = 0; ri < RING_DEFS.length; ri++) {
      ...
      geo.setAttribute(
        "aColor",
        new THREE.BufferAttribute(colArr, 4).setUsage(THREE.DynamicDrawUsage),
      );
      ...
      arcGeo.setAttribute(
        "position",
        new THREE.BufferAttribute(new Float32Array(genRingArcSegments(def)), 3),
      );
    ...
    ashGeo.setAttribute(
      "position",
      new THREE.BufferAttribute(ashPos, 3).setUsage(THREE.DynamicDrawUsage),
    );
    ashGeo.setAttribute(
      "aSize",
      new THREE.BufferAttribute(ashSz, 1).setUsage(THREE.DynamicDrawUsage),
    );
    ashGeo.setAttribute(
      "aOpacity",
      new THREE.BufferAttribute(ashOp, 1).setUsage(THREE.DynamicDrawUsage),
    );
    ...
    deepGeo.setAttribute(
      "position",
      new THREE.BufferAttribute(deepPos, 3).setUsage(THREE.DynamicDrawUsage),
    );
    deepGeo.setAttribute(
      "aSize",
      new THREE.BufferAttribute(deepSz, 1).setUsage(THREE.DynamicDrawUsage),
    );
    deepGeo.setAttribute(
      "aOpacity",
      new THREE.BufferAttribute(deepOp, 1).setUsage(THREE.DynamicDrawUsage),
    );
    ...
    starGeo.setAttribute(
      "position",
      new THREE.BufferAttribute(starPos, 3).setUsage(THREE.DynamicDrawUsage),
    );
    starGeo.setAttribute(
      "aSize",
      new THREE.BufferAttribute(starSz, 1).setUsage(THREE.DynamicDrawUsage),
    );
    starGeo.setAttribute(
      "aOpacity",
      new THREE.BufferAttribute(starOp, 1).setUsage(THREE.DynamicDrawUsage),
    );
    ...
    function tick() {
      ...
      for (let i = 0; i < INTERIOR_DUST_N; i++) {
        ...
        const resonance = Math.sin(
          st.phase * 1.15 + p.x * 2.1 + p.y * 1.4 + p.z * 1.8,
        );
        dustOp[i] =
          p.op *
          (0.72 +
            Math.sin(p.phase) * 0.16 +
            Math.cos(p.phase * 0.7) * 0.07 +
            resonance * 0.07);
      }
      (dustGeo.getAttribute("aOpacity") as THREE.BufferAttribute).needsUpdate =
        true;
      ...
      for (let ri = 0; ri < RING_DEFS.length; ri++) {
        ...
        if (themeChanged) {
          ...
          (
            buf.geo.getAttribute("aColor") as THREE.BufferAttribute
          ).needsUpdate = true;
        }
      }
      ...
      for (let i = 0; i < ASH_N; i++) {
        ...
        if (ptr.active) {
          ...
          if (dist < 2.4 && dist > 0) {
            ...
            a.vx -= (dx / dist) * pull;
            a.vy -= (dy / dist) * pull;
            a.vx += (-dy / dist) * pull * 0.5;
            a.vy += (dx / dist) * pull * 0.5;
          }
        }
        a.x += a.vx;
        a.y += a.vy;
        ...
        ashPos[i * 3 + 2] = a.z;
        ashSz[i] = a.size;
        ashOp[i] = a.baseOp * fadeT * fadeB;
      }
      (ashGeo.getAttribute("position") as THREE.BufferAttribute).needsUpdate =
        true;
      (ashGeo.getAttribute("aSize") as THREE.BufferAttribute).needsUpdate =
        true;
      (ashGeo.getAttribute("aOpacity") as THREE.BufferAttribute).needsUpdate =
        true;
      ...
      for (let i = 0; i < DEEP_FIELD_N; i++) {
        ...
        const twinkle =
          0.72 + Math.sin(p.phase * 1.7) * 0.18 + Math.cos(p.phase * 0.8) * 0.1;

        deepPos[i * 3] = ca * radiusWobble;
        deepPos[i * 3 + 1] = p.y + Math.cos(p.phase * 0.55) * 0.06;
        deepPos[i * 3 + 2] = z;
        deepSz[i] = p.size * (0.68 + depth * 0.24);
        deepOp[i] = p.baseOp * twinkle * centerFade * (0.42 + depth * 0.3);
      }
      (deepGeo.getAttribute("position") as THREE.BufferAttribute).needsUpdate =
        true;
      (deepGeo.getAttribute("aSize") as THREE.BufferAttribute).needsUpdate =
        true;
      (deepGeo.getAttribute("aOpacity") as THREE.BufferAttribute).needsUpdate =
        true;
      ...
      (starGeo.getAttribute("aSize") as THREE.BufferAttribute).needsUpdate =
        true;
      (starGeo.getAttribute("aOpacity") as THREE.BufferAttribute).needsUpdate =
        true;
    ...
    return () => {
      ...
      ringBufs.forEach((b) => {
        ...
        b.arcGeo.dispose();
        b.arcMat.dispose();
        const mesh = b.tiltGroup.children.find(
