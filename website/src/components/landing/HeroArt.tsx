  ...
  useEffect(() => {
    ...
    function tick() {
      ...
      for (let ei = 0; ei < EYE_DEFS.length; ei++) {
        ...
        const entryScaleX = Math.min(
          1.0,
          Math.max(0.01, (eyeEntry - 0.1) * 1.25),
        );
        ...
        if (
          cursor.active &&
          Math.hypot(ptrLogical.x - ct.px, ptrLogical.y - ct.py) <
            logicalSize * 0.95
        )
          nearEyes = true;
      }
      ...
      const gazeX = cursor.active
        ? cursor.x * 6.0
        : Math.sin(pose.phase * 0.8) * 1.5;
      const gazeY = cursor.active
        ? -cursor.y * 6.0
        : Math.cos(pose.phase * 1.2) * 1.5;
