...
data class Connection(val origin: String, val hostToken: String, val nodeToken: String = "")
...
class CredentialStore(context: Context) {
    ...
    fun save(value: Connection) {
        ...
        val plain = JSONObject().put("origin", value.origin).put("hostToken", value.hostToken)
            .put("nodeToken", value.nodeToken).toString().toByteArray(Charsets.UTF_8)
    ...
    fun load(): Connection? {
        ...
        return Connection(json.getString("origin"), json.getString("hostToken"), json.optString("nodeToken"))
    }
