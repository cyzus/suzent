export const en = {
  speech: {
    autoFormat: 'Model default',
    title: 'Voice settings',
    engine: 'Speech engine',
    system: 'System speech (no API)',
    api: 'API speech',
    voice: 'Voice',
    defaultVoice: 'Device default',
    play: 'Play',
    stop: 'Stop',
    unavailable:
      'No local voices are available. Install a system voice or open Suzent on a supported device.',
    failed: 'Unable to complete this action. Check your settings and try again.',
    saved: 'Voice settings saved.',
    systemHelp:
      'Uses local voices on the device playing the message. Audio cannot be exported. Per-call options override these defaults.',
    apiHelp:
      'Uses the TTS model role and saves replayable audio. Voice IDs, formats, speed and style support depend on the provider. Unsupported options return an error.',
    language: 'Language (e.g. zh-CN)',
    format: 'Audio format',
    instructions: 'Tone and style instructions',
    speed: 'Speed',
    pitch: 'Pitch',
    volume: 'Volume',
  },

  mobileAccess: {
    fullAccess: 'Full access',
    restrictedAccess: 'Restricted access',
    fullAccessHelp:
      'Use conversations, send and stop responses, and approve tool requests from your phone.',
    restrictAccess: 'Restrict access',
    restoreFullAccess: 'Restore full access',
    approve_tools: 'Approve tool requests',

    choosePermissions: 'Choose what this phone can access before generating its QR code.',
    cancelInvitation: 'Close QR code and cancel invitation',

    title: 'Mobile access',
    description:
      'Pair the Suzent app with this desktop. Each phone receives its own revocable access.',
    origin: 'Backend address reachable from your phone',
    network:
      'The QR code includes this address plus detected LAN and Tailscale addresses. Your phone checks reachability before you confirm. Release apps require HTTPS; HTTP is debug-only.',
    generate: 'Pair a phone',
    expired: 'Invitation expired. Generate a new QR code.',
    qrAlt: 'Suzent mobile pairing QR code',
    scan: 'Scan in the Suzent app and confirm the connection on your phone. Anyone with this one-time QR code can accept the permissions above.',
    expires: 'Expires in {seconds}s',
    copy: 'Copy pairing invitation',
    compare: 'Confirm that the phone shows code {code}.',
    all_chats: 'Read all conversations, including future ones',
    create_chats: 'Create conversations',
    send: 'Send messages',
    stop: 'Stop responses',
    sharedChats: 'Conversations this phone can read',
    toolPolicy:
      'Sending can run agent tools under the existing conversation policy. With approval access, tool requests can be confirmed on your phone. Node access is enabled separately.',
    approve: 'Approve phone',
    deny: 'Deny',
    revoke: 'Revoke access',
    chatCount: '{count} shared conversations',
    readOnly: 'Read only',
    loadError: 'Could not load mobile access. Refresh to retry.',
    actionError: 'Could not complete the action. Check the address and connection, then retry.',
  },
  console: {
    nav: {
      label: 'Console navigation',
      title: 'Suzent',
      chat: 'Chat',
      ops: 'Operations',
      collapse: 'Collapse sidebar',
      expand: 'Expand sidebar',
    },
    ops: {
      title: 'Operations',
      subtitle: 'The background service on this host, and what it last logged.',
      serviceTitle: 'Background service',
      serviceDesc: 'Runs Suzent without a desktop session, restarting it on crash and at login.',
      status: 'Status',
      uptime: 'Uptime',
      memory: 'Memory',
      notInstalled: 'Not installed',
      ready: 'Ready',
      starting: 'Starting',
      stopped: 'Stopped',
      restart: 'Restart service',
      selfManagedNote:
        'This console is served by the service below, so restarting it will briefly disconnect this page.',
      reconnecting: 'Restarting — waiting for the host to come back…',
      reconnectTimeout:
        'The host did not come back within a minute. Check the log, or the service manager on the host.',
      cannotDisableSelf:
        'This service is serving the console. Turning it off here would leave no way to turn it back on remotely — run "suzent service uninstall" on the host instead.',
      unreachable: 'Could not reach this host. It may be restarting, or the connection is gone.',
    },
    health: {
      checking: 'Checking the connection',
      online: 'Connected',
      offline: 'Not reachable',
    },
  },
  imageTool: {
    previewFailed: 'Image preview unavailable. Open the file to retry.',
  },
  common: {
    ok: 'OK',
    cancel: 'Cancel',
    close: 'Close',
    save: 'Save',
    saving: 'Saving...',
    enabled: 'ENABLED',
    disabled: 'DISABLED',
    add: 'Add',
    remove: 'Remove',
    on: 'ON',
    off: 'OFF',
    loading: 'Loading...',
    error: 'Error',
    retry: 'Retry',
    refresh: 'Refresh',
    go: 'Go',
    edit: 'Edit',
    copy: 'Copy',
    copied: 'Copied',
    unsaved: 'Unsaved',
  },
  app: {
    title: 'Suzent',
    initializing: 'INITIALIZING',
    connectingToCore: 'Connecting to Neural Core...',
    systemIdentity: 'SUZENT / SYSTEM',
    systemStartup: 'System startup',
    backendErrorTitle: 'CONNECTION FAILED',
    backendStartTimeout:
      'Backend startup timed out. Check ~/.suzent/runtime/server.log for details.',
    startingBackend: 'Starting and checking the backend service',
    verifyingBackend: 'Verifying backend compatibility',
    startupElapsed: 'Elapsed {seconds}s',
    logoAriaLabel: 'Suzent logo',
    backendCompatibility: {
      api: 'Frontend/backend API mismatch ({frontend} vs {backend}). Run suzent update.',
      build: 'Frontend/backend build mismatch ({frontend} vs {backend}). Run suzent update.',
      version: 'Frontend/backend version mismatch ({frontend} vs {backend}). Run suzent update.',
      verifyTimeout:
        'Backend version verification timed out. The backend may still be recovering; close and reopen Suzent.',
      verifyFailed: 'Failed to verify backend version: {error}',
    },
  },
  hostPicker: {
    title: 'Choose a folder',
    roots: 'Drives',
    loading: 'Loading...',
    empty: 'No sub-folders here',
    cancel: 'Cancel',
    select: 'Select',
  },
  bootstrap: {
    title: 'Set up Suzent',
    windowTitle: 'Suzent Setup',
    start: 'Install',
    running: 'Installing...',
    changeDir: 'Change',
    chooseInstallDir: 'Choose Suzent install folder',
    installerMissing:
      'Suzent is not installed yet. Download and run the Suzent Installer, then reopen the app.',
    stageFailed: '{stage} failed',
    state: {
      pending: 'Pending',
      running: 'Running',
      succeeded: 'Done',
      skipped: 'Skipped',
      failed: 'Failed',
    },
    stages: {
      git: 'Installing Git',
      uv: 'Installing uv',
      python: 'Verifying Python',
      repository: 'Cloning workspace',
      env: 'Writing environment',
      dependencies: 'Installing dependencies',
      ui: 'Downloading desktop app',
      playwright: 'Installing Chromium',
      shortcuts: 'Creating shortcuts',
      shim: 'Writing launcher',
    },
  },
  nav: {
    chat: 'Chat',
    memory: 'Memory',
    skills: 'Skills',
  },
  views: {
    memorySystem: 'MEMORY SYSTEM',
    skillsLibrary: 'SKILLS LIBRARY',
    robotGallery: 'ROBOT GALLERY',
  },
  sidebar: {
    open: 'Open Sidebar',
    close: 'Close Sidebar',
    tabs: {
      chats: 'Chats',
      config: 'Config',
      files: 'Files',
      tools: 'Tools',
      context: 'Context',
      browser: 'Web',
      canvas: 'Canvas',
      agents: 'Background tasks',
      plan: 'Goal',
      project: 'Board',
    },
    settings: 'Settings',
    settingsDesc: '',
  },
  repositoryContext: {
    title: 'Context',
    description: 'Project memory and effective repository instructions',
    projectContext: 'Project context',
    projectMemory: 'Project memory',
    enlarge: 'Enlarge',
    repositoryInstructions: 'Repository instructions',
    repositoryInstructionsDesc: 'Loaded ancestor-first; the closest file has final precedence.',
    empty: 'No project context or repository instructions found.',
    loadFailed: 'Failed to load repository context',
    saveFailed: 'Failed to save project context',
    sources: {
      repository: 'Repository',
      working: 'Working directory',
    },
  },
  backgroundTasks: {
    heading: 'Background tasks ({count})',
    empty: 'No background tasks in this session',
    shell: 'Shell command',
    output: 'Output (latest)',
    exitCode: 'Exit code: {code}',
    running: '{count} background task(s) running',
    loadFailed: 'Could not load task',
    stopFailed: 'Could not stop task',
  },
  subAgents: {
    model: 'Model',
    heading: 'Sub-agents ({count})',
    empty: 'No sub-agents in this session',
    loading: 'Loading...',
    title: 'Sub-agent',
    delegated: 'Delegated task',
    task: 'Task',
    tools: 'Tools ({count})',
    toolCalls: 'Tool calls ({count})',
    result: 'Result',
    error: 'Error',
    stoppedBecause: 'Stopped',
    live: 'Live',
    stop: 'Stop',
    stopping: 'Stopping...',
    close: 'Close',
    viewLog: 'View log',
    activity: 'Working on',
    thinking: 'Thinking…',
    responding: 'Writing its answer…',
    steerPlaceholder: 'Redirect this sub-agent…',
    steer: 'Redirect',
    steerFailed: 'Could not reach this sub-agent — it may have just finished.',
    steerQueued: 'Queued',
    steerQueuedHint: 'applies after the current step',
    steerTaken: 'Picked up',
    working: 'Working...',
    contextForked: 'Context forked',
    worktreeIsolated: 'Worktree isolated',
    worktree: 'Worktree',
    branch: 'Branch',
    path: 'Path',
    toolCount: '{count} tools',
    status: {
      queued: 'Queued',
      running: 'Running',
      completed: 'Done',
      failed: 'Failed',
      cancelled: 'Stopped',
    },
  },
  titlebar: {
    minimize: 'Minimize',
    maximize: 'Maximize',
    restore: 'Restore',
    close: 'Close',
  },
  updates: {
    sectionTitle: 'Software updates',
    checkDescription: 'Check GitHub Releases for a newer Suzent version.',
    check: 'Check for updates',
    checking: 'Checking...',
    upToDate: 'Suzent is up to date.',
    checkFailed: 'Could not check for updates: {error}',
    available: 'Update available',
    availableTitle: 'Update available: {version}',
    updateNow: 'Update',
    starting: 'Updating...',
    latestVersion: 'latest version',
    confirmRestart: 'Suzent will close, update to {version}, and restart automatically. Continue?',
    startFailed: 'Failed to start update: {error}',
  },
  config: {
    loading: 'Loading config...',
    agentLabel: 'Agent',
    agent: {
      code: '📝 Writes and executes Python code',
      toolcalling: '🔧 Direct tool calling without code',
    },
    toolsLabel: 'Tools',
    toolsEmpty: 'No tools available',
    toolCatalog: {
      approval: 'Approval',
      builtin: 'Built-in',
      builtinHint: 'Built-in tool — always equipped and cannot be turned off.',
      off: 'Off',
      offHint: 'Turned off — the agent cannot use this one, and cannot find it by searching.',
      searchableNote:
        'Unchecked tools are not loaded up front, but the agent can still find and load most of them mid-task. Ones marked OFF are the exception.',
      deactivate: 'Deactivate',
      deactivateTool: 'Deactivate {name}',
      capabilityAria: '{name} capability',
    },
    memory: {
      label: 'Memory System',
      button: 'Memory Tools',
      enabledDesc: '✓ Agent can search and update memory',
      disabledDesc: 'Memory tools are disabled',
    },
    sandbox: {
      label: 'Sandbox System',
      button: 'Sandbox Execution',
      enabledDesc: '✓ Agent can run code in isolated sandbox',
      disabledDesc: 'Sandbox execution is disabled',
    },
    mcp: {
      label: 'MCP Servers',
      name: 'Name',
      url: 'URL',
      stdio: 'Stdio',
      enableServer: 'Enable server',
      noneConfigured: 'No MCP servers configured.',
      manageInSettings: 'Manage in Settings',
      noneConfiguredManageInSettings: 'No MCP servers configured. Add servers in Settings.',
      enabledUrls: 'ENABLED: {count} URL(S)',
      envPrefix: 'env:{json}',
      urlPlaceholder: 'https://host/path',
      commandPlaceholder: 'command',
      argsPlaceholder: 'args',
      envPlaceholder: 'env',
    },
  },
  settings: {
    network: {
      restartRequiredManually:
        'Saved. Restart the Suzent backend for the new network binding to take effect.',
    },
    browser: {
      extension: 'Use my browser (extension)',
      extensionConnected: 'Connected',
      extensionDisconnected: 'Not connected',
      extensionHelp:
        'Use the tabs and logins in the browser where you install the extension. No remote-debugging setup is needed. Suzent can control web tabs while connected; disconnect here or in the extension to stop access.',
      extensionInstall:
        'Install Suzent Browser from your browser\u2019s store, then pair your browser.',
      extensionStore: 'Open {store}',
      extensionStoreChrome: 'Chrome Web Store',
      extensionStoreEdge: 'Edge Add-ons',
      extensionSource: 'Local extension folder (select this folder when loading unpacked)',
      extensionPair: 'Open the pairing page in the browser where you installed the extension.',
      extensionDownload: 'Download extension',
      extensionPairButton: 'Pair browser',
      extensionRevoke: 'Disconnect and forget',
      extensionOtherBrowser:
        'Different browser? Paste this private link there. Expires in 5 minutes.',

      connectionMode: 'Connection mode',
      managed: 'Launch a managed browser',
      existing: 'Direct connection (advanced)',
      existingHelp:
        'Select Chrome or Edge, open its inspect page, and enable remote debugging. Approve the browser connection prompt. Suzent starts in a new tab; ask the agent to list and select your open tabs. Your browser stays open when Suzent disconnects.',

      title: 'Browser',
      subtitle: 'Choose how Suzent connects to a browser.',
      channel: 'Browser to use',
      chromium: 'Bundled Chromium',
      chrome: 'Google Chrome',
      edge: 'Microsoft Edge',
      channelHelp:
        'Chrome or Edge must already be installed on the computer running Suzent’s backend.',
      chromiumHelp:
        'Downloaded automatically if needed; internet access is required for installation.',
      notInstalled: 'Not found in a supported installation location',
      recheck: 'Refresh',
      selectedUnavailable:
        'The selected browser was not found. Choose Chromium or install this browser, then check again.',
      persistent: 'Remember browser logins',
      persistentHelp: 'Keep logins in a separate Suzent profile.',
      visible: 'Show browser window',
      visibleHelp: 'Show the window to sign in or browse alongside Suzent.',
      overrides:
        'Disabled settings are controlled by environment variables. Remove those overrides to change them here.',
      restart: 'Applies on the next browser action.',
      saved: 'Saved · applies on the next action.',
      saving: 'Saving…',
      loading: 'Loading browser settings…',
      error: 'Could not load or save browser settings. Check the backend connection and try again.',
      retry: 'Try again',
      extensionInstallTitle: 'Install extension',
      extensionPairTitle: 'Connect browser',
      extensionReady: 'Ready to use your tabs and logins. Ask Suzent to select a tab.',
      extensionDetails: 'Setup help & options',
      extensionDownloadHelp:
        'For manual installation, use the folder below or extract the ZIP to a permanent folder. Open Extensions → Developer mode → Load unpacked.',
      directSetup: 'Remote debugging setup',
    },
    title: 'Settings',
    category: 'Category',
    language: 'Language',
    usingSuzent: 'Using Suzent',
    groups: {
      agent: 'Agent',
      connections: 'Connections',
      application: 'Application',
    },
    memoryConfig: {
      title: 'Memory Configuration',
      subtitle: 'Manage background consolidation and the notebook vault',
      systemConfigTitle: 'System Configuration',
      systemConfigDesc: 'Configure the notebook mount path for your workspace.',
      globalMemory: 'Global Memory',
      modelsLabel: 'Models',
      editInModelRoles: 'Model Roles',
      embeddingModelLabel: 'Embedding model',
      extractionModelLabel: 'Extraction model',
      modelNotConfigured: 'Not configured',
      dreamTitle: 'Dreaming agent',
      dreamDesc: 'Background memory consolidation that folds daily logs into the notebook vault.',
      consolidatedThrough: 'Consolidated through',
      pendingLogs: 'Pending logs',
      pendingFacts: 'Pending facts',
      pendingConfirmations: 'Pending confirmations',
      backlogProgress: 'Backlog progress',
      progressCount: '{current}/{total} logs',
      complete: 'complete',
      lastRun: 'Last run',
      lastResult: 'Last result',
      nextBatch: 'Next batch',
      runNow: 'Run now',
      runIngest: 'Run ingest',
      runLint: 'Run lint',
      runningNow: 'Running...',
      never: 'Never',
      noRunsYet: 'No runs yet',
      lastResultSkipped: 'Skipped after repeated no-op attempts',
      lastResultAdvanced: 'Advanced through {watermark}',
      lastResultNotAdvanced: 'No durable page changes',
      lastResultRan: 'Run completed',
      ingestPhase: 'Ingest',
      lintPhase: 'Lint',
      lintTitle: 'Weekly lint',
      lintEvery: 'Cadence',
      lintLastRun: 'Last lint',
      lintDue: 'Due now',
      lintScheduled: 'Up to date',
      lintNever: 'Not yet run',
      lintEveryDays: 'every {days}d',
      statusBarIdle: 'Memory idle',
      statusBarNeedsReview: 'Memory needs review',
      statusBarPending: 'Memory {count} pending',
      statusBarWatermark: 'Consolidated through {watermark}',
      dreamStatus: {
        loading: 'Loading',
        error: 'Needs review',
        inactive: 'Inactive',
        disabled: 'Disabled',
        preparing: 'Preparing...',
        running: 'Dreaming...',
        finalizing: 'Finalizing...',
        pending: 'Pending',
        idle: 'Idle',
      },
    },
    providers: {
      searchProviders: 'Search providers...',
      noMatchingProviders: 'No matching providers. Try another name.',
      searchModels: 'Search models by name or ID...',
      enabledCount: '{count} models enabled',
      keySet: 'Key configured',
      noKey: 'Key needed',
      customProvider: 'Custom provider',
      environmentValue: 'Environment',
      savedValue: 'Saved',
      testConnection: 'Test connection & fetch models',
      connectionHint:
        'Save your credentials, then test the connection and choose which models to enable.',
      connectionSuccess:
        'Connection successful. Found {count} models. Select the models you want to use.',
      unsavedCredentials: 'Save or cancel your credential edits before testing the connection.',
      enabledSummary: '{models} enabled models · {providers} providers',
      enabledSummaryHint:
        'Enabled models are not necessarily connected. Select a model to open its provider settings.',
      enabledEmpty: 'No models enabled yet. Expand a provider below to get started.',
      title: 'Model Providers',
      subtitle: 'Connect provider API keys and manage available models',
      apiKeysTab: 'API keys',
      modelsTab: 'Models',
      addModelIdPlaceholder: 'Add model ID...',
      fetch: 'Fetch',
      fetching: 'Fetching...',
      noModelsFound: 'No models found',
      fetchModels: 'Fetch models',
      modelsAvailable: '{count} available',
      noCredentials: 'No credential fields — configure via environment variables.',
      changeKey: 'Change',
      showKey: 'Show',
      hideKey: 'Hide',
      addProvider: '+ Add Provider',
      deleteProvider: 'Remove provider',
      sync: 'Sync Capabilities',
      syncing: 'Syncing...',
      syncTooltip: 'Pull pricing, context windows, and capability flags from LiteLLM',
      syncDone: 'Synced {providers} providers, {models} models',
      syncFailed: 'Sync failed',
      chatgpt: {
        title: 'ChatGPT Subscription',
        subtitle: 'Sign in with your ChatGPT account',
        loadingStatus: 'Checking login status...',
        signIn: 'Sign In',
        disconnect: 'Disconnect',
        awaitingAuth: 'Visit the URL below and enter the code to sign in:',
        waiting: 'Waiting for authorization...',
        status: {
          connected: 'Connected',
          not_logged_in: 'Not Signed In',
          token_expired: 'Expired',
        },
      },
      addForm: {
        title: 'Add Custom Provider',
        id: 'Provider ID',
        label: 'Display Name',
        apiType: 'API Type',
        baseUrl: 'Base URL (optional)',
        apiKeySection: 'API Key Field (optional)',
        envVar: 'Env Variable',
        fieldLabel: 'Field Label',
        placeholder: 'Placeholder',
        requiredFields: 'Provider ID and Display Name are required.',
        save: 'Add Provider',
      },
    },
    mcp: {
      title: 'MCP Servers',
      subtitle: 'Connect external tools and resources via MCP',
      addNewServerTitle: 'Add new server',
      addNewServerDesc: 'Add a new MCP server to connect external tools and resources.',
      nameOptionalPlaceholder: 'Name (optional)',
      headersPlaceholder: 'Headers (Header-Name=value, X-Api-Key=abc123)',
      commandPlaceholder: 'Command (e.g., uv, npx, python)',
      argsPlaceholder: 'Arguments (comma-separated: run, mcp-server, --option)',
      envPlaceholder: 'Environment vars (KEY=value, KEY2=value2)',
      adding: 'Adding...',
      addServer: 'Add Server',
      configuredServersTitle: 'Configured Servers',
      configuredServersDesc:
        'Manage your MCP servers. You can enable/disable servers for the current session.',
      noServersConfigured: 'No servers configured yet.',
      test: 'Test',
      testing: 'Testing...',
      reachable: 'Reachable',
      unreachable: 'Unreachable',
      editServerTitle: 'Edit Server',
      saveServer: 'Save Changes',
      saving: 'Saving...',
      transportLabel: 'Transport',
      toggle: 'Toggle',
      commandLabel: 'Command',
      stdinMode: 'stdin mode',
    },
    acp: {
      title: 'ACP Agents',
      subtitle: 'Local agent processes that speak the Agent Client Protocol',
      registeredTitle: 'Registered Agents',
      registeredDesc:
        'Built-in and user-defined ACP agents. Add custom agents in ~/.suzent/acp_agents.json.',
      noAgents: 'No ACP agents registered.',
      ready: 'Ready',
      notInstalled: 'Not installed',
      probe: 'Probe',
      probing: 'Probing...',
      probeOk: 'Reachable',
      probeFailed: 'Unreachable',
      copyInstall: 'Copy Install',
      copyLogin: 'Copy Login',
      copied: 'Copied!',
      copyCommand: 'Copy',
      docs: 'Docs',
      setupTitle: 'Setup Required',
      installGuide: 'Install Guide',
      installHint:
        'Install routes differ by platform. Follow the official guide, then reopen this tab.',
      adapterTitle: 'Adapter Package',
      adapterHint:
        'Suzent launches this agent through an npm adapter. Installing it up front keeps the first message from waiting on a download.',
      builtIn: 'Built-in',
      custom: 'Custom',
      command: 'Command',
      activeSessions: '{count} active',
      savedSessions: '{count} saved',
      noSessions: 'No sessions',
      authOk: 'Authenticated',
      authUnknown: 'Auth unknown',
      authMissing: 'Auth required',
    },
    categories: {
      providers: 'Providers',
      roles: 'Model Roles',
      memory: 'Memory System',
      security: 'Security',
      social: 'Social Channels',
      devices: 'Devices',
      mesh: 'Mesh',
      mcp: 'MCP Servers',
      acpAgents: 'ACP Agents',
      automation: 'Automation',
      service: 'Background Service',
      data: 'Data & Sync',
      usage: 'Usage',
      appearance: 'Appearance',
      about: 'About',
    },
    service: {
      title: 'Background Service',
      subtitle:
        'Keep automations, channels, and background tasks running when the window is closed',
      backgroundTitle: 'Run Suzent in the background',
      backgroundDesc:
        'Starts Suzent when you sign in and keeps it available after the desktop window closes.',
      status: 'Status',
      uptime: 'Uptime',
      memory: 'Memory',
      ready: 'Ready',
      starting: 'Starting',
      stopped: 'Stopped',
      notInstalled: 'Not installed',
      restart: 'Restart service',
      copyLogPath: 'Copy log path',
      refresh: 'Refresh',
      logTitle: 'Service log',
      logDesc: 'The tail of server.log, which is the only account of a headless start-up.',
      logLines: 'Lines',
      follow: 'Follow',
      logMissing: 'No log file yet. It appears once the service has run.',
      logLevel: 'Level',
      logLevelAll: 'All',
      logSearch: 'Search log',
      logCopy: 'Copy',
      logCopied: 'Copied',
      logEmpty: 'The log is empty.',
      logNoMatch: 'No lines match this filter.',
      logCounts: '{shown} of {total} lines',
      logProblems: '{count} errors',
    },
    security: {
      title: 'Security',
      subtitle: 'Sandbox and execution isolation settings',
    },
    data: {
      title: 'Data',
      subtitle: 'Sync portable config, skills, and memory across devices',
      githubTitle: 'GitHub Sync',
      githubDesc:
        'Share portable config, skills, and markdown memory through a local Git repo with a GitHub remote. Chats, runtime data, indexes, and plaintext secrets stay local.',
      githubQuickStartTitle: 'Quick start',
      githubQuickStartDesc:
        'Create a private GitHub repo (default suzent-brain) and connect. Sign in with GitHub once — no CLI or personal access token needed.',
      githubQuickStartDefaultPath: 'Default folder: {path}',
      githubQuickStartRepoPlaceholder: 'Optional: your-username/suzent-brain or GitHub clone URL',
      githubQuickStartButton: 'Quick start',
      githubQuickStartRunning: 'Setting up GitHub sync...',
      githubQuickStartDone: 'Connected to {repo}. {summary}',
      githubRepoNameLabel: 'GitHub repository name',
      githubRepoNameHint:
        'Creates or links username/{name} on GitHub (private). You can rename it before starting.',
      githubSignInButton: 'Sign in with GitHub',
      githubCheckingAuth: 'Checking GitHub sign-in',
      githubSignInDesc: 'Authorize Suzent to access your GitHub repositories.',
      githubSignInCode: 'Open {url} and enter this code:',
      githubSignInWaiting: 'Waiting for authorization…',
      githubSignInExpired: 'Authorization code expired. Click Sign in to try again.',
      githubSignInDenied: 'Authorization was denied.',
      githubSignInDone: 'Signed in as {username}',
      githubSignOut: 'Sign out',
      githubAdvanced: 'Advanced options',
      githubConnected: 'Connected',
      githubSignedIn: 'GitHub signed in',
      githubRepoPlaceholder: 'Local Git repo folder (advanced)',
      githubBranch: 'Branch',
      githubRemote: 'Remote',
      githubAutoSync: 'Auto-sync',
      githubAutoResolve: 'Agent conflict resolve',
      githubInterval: 'Hours',
      githubFileOnlyScope:
        'Syncs config, skills, and memory. API keys and credentials stay on this device.',
      githubNotConfigured: 'GitHub sync is not configured.',
      githubCloudApplied: 'Cloud changes pulled and applied as the source of truth.',
      githubReviewTitle: 'Source Control: GitHub Sync',
      githubReviewSubtitle: 'Review pending {operation} before applying protected changes',
      githubReviewSync: 'sync',
      githubReviewAdded: 'Added',
      githubReviewModified: 'Modified',
      githubReviewDeleted: 'Deleted',
      githubReviewOutgoing: 'Outgoing Changes',
      githubReviewIncoming: 'Incoming Changes',
      githubReviewCategoryConfig: 'config',
      githubReviewCategorySkills: 'skills',
      githubReviewCategoryMemory: 'memory',
      githubReviewCategorySync: 'sync',
      githubReviewCategoryOther: 'other',
      githubReviewDiff: 'Diff preview',
      githubReviewNoDiff: 'No textual diff available',
      githubReviewLoadingDiff: 'Loading diff…',
      githubReviewFilesHidden: '{count} more files hidden',
      githubReviewHighRisk: '{count} high risk',
      githubReviewDiscard: 'Discard Outgoing',
      githubReviewDiscardTitle:
        'Restore local synced files from the current cloud payload and clear outgoing changes',
      githubReviewDiscardFile: 'Discard',
      githubReviewDiscardFileTitle: 'Restore this local file from the committed sync payload',
      githubDiscardedOutgoing: 'Discarded {count} outgoing sync changes.',
      githubNoOutgoing: 'No outgoing sync changes to discard.',
      githubDiscardedFile: 'Discarded local change to {path}.',
      githubReviewPull: 'Pull Cloud',
      githubReviewPullTitle:
        'Pull cloud changes and let cloud synced content replace local synced content',
      githubReviewConfirm: 'Confirm {operation}',
      githubSave: 'Save profile',
      githubValidate: 'Validate',
      githubSaveAuto: 'Save auto',
      githubPush: 'Push',
      githubPull: 'Pull',
      githubPushCount: 'Push ({count})',
      githubPullCount: 'Pull ({count})',
      githubPushTitle: 'Push {count} outgoing changes',
      githubPullTitle: 'Pull {count} incoming updates',
      githubSaved: 'GitHub sync settings saved.',
      githubValid: 'GitHub repository validated.',
      githubPushed: 'GitHub sync payload pushed.',
      githubPulled: 'GitHub sync payload pulled.',
      githubFailed: 'GitHub sync failed: {error}',
      githubPullConfirm:
        'Pull from GitHub sync and replace local portable config, skills, and markdown memory?',
    },
    roles: {
      title: 'Model Roles',
      subtitle:
        'Assign models by task. Empty roles inherit their parent role; explicit assignments override inheritance.',
      customStatus: 'Custom',
      inheritedStatus: 'Inherited',
      restoreInheritance: 'Use inherited models',
      overrideCount: '{count} overrides',
      defaultsGroup: 'Default models',
      defaultsGroupDesc:
        'Set Primary first. Lightweight tasks inherit Cheap; review tasks share Decision.',
      tasksGroup: 'Background tasks',
      tasksGroupDesc:
        'Title and memory extraction inherit Cheap. Dream inherits Primary. Override only where needed.',
      specialistsGroup: 'Specialist models',
      specialistsGroupDesc:
        'Vision can inherit a compatible Primary model. Other capabilities require their own models.',
      modelOrder: 'Models are ordered by priority, with your first choice at the top.',
      compactIntro: 'Choose a role to edit. Tasks use inherited models until you assign their own.',
      fromRole: 'From {role}',
      chooseModel: 'Choose a model',
      inheritPreview: 'Use {model} from {role}.',
      emptyPreview: 'No model selected. Add one below to enable this role.',
      applyChanges: 'Apply changes',
      modelSource: 'Model source',
      inheritMode: 'Inherit from parent',
      customMode: 'Choose own model',
      parentMissing:
        'The parent has no compatible model yet. Configure it first, or choose your own model.',
      clearRole: 'Clear model assignment',
      backupModels: 'Backup models · {count} (optional)',
      addBackup: 'Add backup model',
      primary: 'Primary',
      primaryDesc: 'Main chat agent. Needs tool use and large context.',
      cheap: 'Cheap',
      cheapDesc: 'Default for lightweight tasks and decisions.',
      decision: 'Decision',
      decisionDesc: 'Shared model for goal and permission decisions.',
      autoTitle: 'Title',
      autoTitleDesc: 'Generate conversation titles.',
      memoryExtraction: 'Memory Extraction',
      memoryExtractionDesc: 'Extract durable facts from conversations.',
      dream: 'Dream',
      dreamDesc: 'Consolidate memories and audit the knowledge vault.',
      goalJudge: 'Goal Judge',
      goalJudgeDesc: 'Decide whether an active goal is complete.',
      permissionReview: 'Permission Review',
      permissionReviewDesc: 'Review proposed tool actions for automatic approval.',
      inheritedModels: 'Inherited: {models}',
      inheritsCheap: 'Inherits Cheap → Primary while empty.',
      inheritsDecision: 'Inherits Decision → Cheap → Primary while empty.',
      advancedDecision: 'Advanced decision roles',
      advancedDecisionDesc:
        'Override individual decision tasks. Leave empty to share the Decision model.',
      vision: 'Vision',
      visionDesc: 'Image analysis. Must support multimodal input.',
      embedding: 'Embedding',
      embeddingDesc: 'Vector search and memory retrieval.',
      imageEdit: 'Image Editing',
      videoGeneration: 'Video generation',
      videoGenerationDesc: 'Create videos from text or a reference image. Requires a video model.',
      imageEditDesc:
        'Edit existing images using instructions and optional reference images or masks.',
      imageGeneration: 'Image Generation',
      imageGenerationDesc: 'Text-to-image generation.',
      tts: 'TTS',
      ttsDesc: 'Text-to-speech synthesis.',
      modelsPlaceholder: 'model/name (one per line)',
      notConfigured: 'Not configured',
      fallbackHint: 'Enter one model ID per line. First line is primary; rest are fallbacks.',
      addFromAvailable: 'Add model...',
      customPlaceholder: 'provider/model-name',
      searchPlaceholder: 'Search or type provider/model-name...',
      noModelsFound: 'No models found',
      addCustom: 'Add "{id}"',
      moveUp: 'Move up (higher priority)',
      moveDown: 'Move down (lower priority)',
      firstChoice: 'First choice',
      fallbackNumber: 'Fallback {number}',
      lastResort: 'Last resort',
      chainStops:
        'If every model in this chain fails, this role stops. It does not switch to another role.',
      inheritsPrimary: 'Uses the complete Primary chain while this role is empty.',
      inheritsVisionPrimary:
        'Uses only confirmed vision-capable models from Primary. Models with unknown capabilities are skipped.',
      noImplicitFallback:
        'No implicit fallback. This feature remains unavailable until a model is assigned.',
      unverified: 'Unverified',
      explicitOverride:
        'Not in the verified suggestions for this role. The explicit assignment is still allowed.',
      unregisteredAvailable:
        'Capability-unverified enabled models are also available as explicit overrides.',
    },
    sandbox: {
      title: 'Notebook',
      subtitle: 'Pick one folder to use as /mnt/notebook for all chats.',
      mountTarget: 'Container Path',
      hostFolder: 'Host Folder',
      notConfigured: 'Not configured',
      chooseFolder: 'Choose Folder',
      saveHint: 'Changes are applied automatically.',
    },
    appearance: {
      title: 'Appearance',
      subtitle: 'Choose how Suzent looks and reads',
      languageDesc: 'Choose the language used throughout the application.',
      theme: 'Theme',
      themeDesc: 'Light, dark, or whatever this machine is set to.',
      modes: { light: 'Light', dark: 'Dark', system: 'System' },
      colorScheme: 'Color Scheme',
      colorSchemeDesc: 'Choose the accent palette used in light and dark mode.',
      schemes: { warm: 'Warm', cold: 'Cold', green: 'Green' },
    },
    about: {
      title: 'About',
      subtitle: 'Version information for this Suzent installation',
      description: 'Your personal AI agent, running locally on your desktop.',
      frontendVersion: 'Desktop frontend',
      backendVersion: 'Python backend',
      unavailable: 'Unavailable',
      developmentBuild: 'Development build',
    },
    usage: {
      title: 'Usage',
      subtitle: 'Track your LLM API spending and token usage',
      totalSpend: 'Total Spend',
      avgDaily: 'Avg. Daily',
      totalCalls: 'API Calls',
      totalTokens: 'Tokens',
      lastNDays: 'Last {count} days',
      perDay: 'per day',
      apiCalls: 'API calls',
      days: '{count}d',
      dailySpend: 'Daily Spend',
      today: 'Today',
      tooltipCalls: '{count} calls',
      tokenBreakdown: 'Token Breakdown',
      inputTokens: 'Input Tokens',
      outputTokens: 'Output Tokens',
      tokenActivity: 'Token Activity',
      last24Hours: 'Last 24 hours',
      utcTime: 'UTC',
      tokensLowercase: 'tokens',
      activePeriods: '{count} active periods',
      activityCellLabel: '{date}, {tokens} tokens, {calls} calls',
      less: 'Less',
      more: 'More',
      noUsageYet: 'No usage data yet. Start chatting to see your spending here.',
    },
    saveChanges: 'Save Changes',
    saving: 'Saving...',
    verifyFailed: 'Verification failed or no models found.',
    social: {
      channelEnabled: 'Enabled',
      channelDisabled: 'Disabled',
      searchChannels: 'Search channels…',
      noMatchingChannels: 'No matching channels.',
      connectionTab: 'Connection',
      accessTab: 'Access control',
      accessHint:
        'Enter the user IDs allowed to chat with this channel, separated by commas. Manage pairing requests below.',

      fields: {
        token: 'Bot token',
        app_id: 'App ID',
        app_secret: 'App secret',
        encrypt_key: 'Encryption key',
        verification_token: 'Verification token',
        app_token: 'App token',
        bot_token: 'Bot token',
        base_url: 'Service URL',
        channel_version: 'Channel version',
        get_updates_buf: 'Update cursor',
      },
      enabledCount: '{enabled} / {total} enabled',

      title: 'Social Channels',
      subtitle: 'Configure social platform agents and access control',
      socialModel: 'Social Model',
      useDefaultModel: 'Use Default System Model',
      selectModelPlaceholder: 'SELECT MODEL...',
      agentCapabilitiesTitle: 'Agent Capabilities',
      agentCapabilitiesDesc: 'Configure which tools the social agent can use.',
      enableMemoryTools: 'Enable Memory Tools',
      tools: 'Tools',
      allTools: 'All Tools',
      custom: 'Custom',
      noToolsAvailable: 'No tools available',
      mcpServers: 'MCP Servers',
      systemDefault: 'System Default',
      enable: 'Enable',
      allowedUsersSpecific: 'Allowed users',
      allowedUsersSpecificPlaceholder: 'user_id_1, user_id_2...',
      pairingTitle: 'Pairing & Access Control',
      pairingDesc: 'Require strangers to verify before chatting',
      pairingDisabled: "Pairing disabled — only users in a platform's allowlist can chat.",
      noPendingRequests: 'No pending requests.',
      approve: 'Approve',
      deny: 'Deny',
      wechatAuthTitle: 'WeChat Authentication',
      wechatAuthDesc: 'Scan a QR code to connect a personal WeChat bot.',
      wechatConnected: 'WeChat token is configured. You can reconnect to replace it.',
      wechatLogin: 'Log in with WeChat',
      wechatStarting: 'Starting...',
      wechatWaiting: 'Scan this QR code in WeChat, then keep this panel open.',
      wechatStatus: 'Status',
      wechatQrAlt: 'WeChat login QR code',
    },
    automation: {
      title: 'Automation',
      subtitle: 'Schedule recurring agent jobs and manage the scheduler',
      failedToCreateJob: 'Failed to create job',
      failedToUpdateJob: 'Failed to update job',
      defaultModel: 'Default',
      schedulerStatusTitle: 'Scheduler Status',
      heartbeatToolsTitle: 'Heartbeat Tool Approvals',
      heartbeatToolsDesc:
        'Tools that are automatically approved when heartbeat runs. Leave on All Tools to approve everything, or pick Custom to restrict.',
      noToolsSelected: 'No tools selected',
      running: 'Running',
      stopped: 'Stopped',
      activeOfTotal: '{active} active / {total} total jobs',
      heartbeatTitle: 'Heartbeat',
      heartbeatDesc: 'Periodic agent check-in using',
      heartbeatChecklist: 'checklist.',
      heartbeatRunning: 'Running every {minutes}m in a persistent session.',
      heartbeatDisabled: 'Disabled. Enable to start periodic check-ins.',
      heartbeatCreate: 'Create /shared/HEARTBEAT.md to enable.',
      disable: 'Disable',
      runNow: 'Run Now',
      collapse: 'collapse',
      heartbeatMdEdit: 'edit',
      heartbeatIntervalLabel: 'Interval',
      heartbeatMinutesUnit: 'min',
      lastRun: 'Last run:',
      result: 'Result:',
      resultOk: 'OK (nothing needed attention)',
      errorLabel: 'Error:',
      addNewJobTitle: 'Add New Job',
      addNewJobDesc: 'Schedule a prompt to run automatically on a repeating schedule.',
      jobNamePlaceholder: 'Job name',
      cronExprPlaceholder: 'Cron expr (e.g. */5 * * * *)',
      repeat: 'Repeat',
      freqMinutes: 'Every few minutes',
      freqHourly: 'Hourly',
      freqDaily: 'Daily',
      freqWeekly: 'Weekly',
      freqMonthly: 'Monthly',
      freqAdvanced: 'Advanced (cron)',
      intervalLabel: 'Minutes',
      atMinuteLabel: 'At minute',
      atTimeLabel: 'At time',
      onDaysLabel: 'On days',
      onDayOfMonthLabel: 'Day of month',
      weekdaysShort: 'Weekdays',
      everyDayShort: 'Every day',
      weekdays: 'Sunday,Monday,Tuesday,Wednesday,Thursday,Friday,Saturday',
      advancedHint:
        'Five fields: minute hour day-of-month month day-of-week. Leave a field as * for "any".',
      summaryMinutes: 'Runs every {n} minutes',
      summaryOnce: 'Runs once at {time}',
      freqOnce: 'Once',
      runAtLabel: 'At',
      timezone: 'Timezone',
      tzLocal: 'Machine local time',
      jitterLabel: 'Jitter (sec)',
      catchUp: 'After downtime',
      catchUpSkip: 'Skip missed runs',
      catchUpRunOnce: 'Make up one run',
      runsIn: 'Runs in',
      runsInIsolated: 'Its own isolated chat',
      runsInHint:
        'A bound task sees that conversation and can continue it, at the cost of its history in tokens on every run.',
      quietWhenNothing: 'Stay quiet if nothing to report',
      heartbeatManagedHint: "Managed from this chat's Config tab — edit the heartbeat there.",
      summaryOnceUnset: 'Runs once (time not set)',
      boundTo: 'in {chat}',
      summaryHourly: 'Runs hourly at :{m}',
      summaryDaily: 'Runs daily at {time}',
      summaryWeekly: 'Runs every {day} at {time}',
      summaryMonthly: 'Runs on day {day} of each month at {time}',
      promptPlaceholder: 'Prompt to execute on schedule...',
      announce: 'Announce',
      silent: 'Silent',
      delivery: 'Delivery',
      model: 'Model',
      active: 'Active',
      creating: 'Creating...',
      addJob: 'Add Job',
      scheduledJobsTitle: 'Scheduled Jobs',
      scheduledJobsDesc: 'Manage your automated tasks. Toggle, trigger, edit, or remove jobs.',
      noCronJobs: 'No cron jobs configured yet.',
      cronExpression: 'Cron expression',
      run: 'Run',
      nextRun: 'Next run:',
      hideHistory: 'Hide History',
      showHistory: 'Show History',
      noRunHistory: 'No run history yet.',
    },
  },
  memoryView: {
    initializing: 'Initializing memory system...',
    systemError: 'System error',
    retryConnection: 'Retry connection',
    coreMemoryTitle: 'Core memory files',
    coreMemoryDesc: 'Always-visible memory and non-empty project context',
    projectContextTitle: 'Project context',
    projectContextMeta: '{name} · {count} chats',
    archivalTitle: 'Archival database',
    archivalDesc: 'Read-only search index',
    collapseSection: 'Collapse section',
    expandSection: 'Expand section',
    indexingArchives: 'Indexing archives',
    waiting: 'Waiting',
    tabs: {
      overview: 'Overview',
      dreaming: 'Dreaming',
      dailyLogs: 'Daily Logs',
      memoryFile: 'MEMORY.md',
      transcripts: 'Transcripts',
    },
  },
  browser: {
    exitControl: 'Exit control (Esc)',
    livePreview: 'Live preview',
    stopPreview: 'Stop preview',
    previewOff: 'Preview is off',
    waitingForStream: 'Waiting for preview',
    showTab: 'Show tab',
    showTabError: 'Could not show the browser tab.',
    noSelectedTab: 'No tab selected',
    title: 'Browser',
    navigate: 'Navigate',
    execute: 'Execute',
    executeHint: 'Execute a browser command to start...',
    streamAlt: 'Browser stream',
    status: {
      connected: 'connected',
      connecting: 'connecting',
      disconnected: 'disconnected',
    },
    takeControl: 'Take Control',
    liveControlActive: 'Live Control Active',
  },
  dailyLogs: {
    title: 'Daily logs',
    entries: '{count} entries',
    loading: 'Loading daily logs...',
    loadLogFailed: 'Failed to load log',
    loadDatesFailed: 'Failed to load daily logs',
    emptyTitle: 'No daily logs yet',
    emptyDesc: 'Daily memory logs will appear here as the agent extracts facts from conversations.',
    today: 'Today',
    yesterday: 'Yesterday',
    daysAgo: '{count} days ago',
  },
  memoryFile: {
    fileName: 'MEMORY.md',
    loadFailed: 'Failed to load MEMORY.md',
    loading: 'Loading MEMORY.md...',
    headerWithSize: '{size} • CURATED_LONG_TERM',
    notCreatedYet: 'NOT_CREATED_YET',
    reindex: 'Reindex',
    reindexing: 'Reindexing...',
    reindexSuccess: 'Reindex completed successfully',
    reindexFailed: 'Reindex failed',
    emptyTitle: 'No MEMORY.md yet',
    emptyDesc:
      'The curated long-term memory file will be created automatically as the agent processes conversations and extracts key facts.',
  },
  transcripts: {
    title: 'Session transcripts',
    desc: 'JSONL conversation log',
    sessionLabel: 'Session',
    selectPlaceholder: 'Select a session...',
    lastNLabel: 'Last N entries',
    all: 'All',
    entriesCount: '{count} entries',
    lastN: 'last {n}',
    loading: 'Loading transcript...',
    loadFailed: 'Failed to load transcript',
    selectTitle: 'Select a session',
    selectDesc: 'Choose a session from the dropdown above to view its conversation transcript.',
    emptyTitle: 'No transcript data',
    emptyDesc:
      'This session does not have a transcript yet. Transcripts are created when JSONL transcript logging is enabled.',
    actionsCount: '{count} actions',
  },
  chat: {
    newChat: 'New chat',
    send: 'Send',
    placeholder: 'Type a message...',
    runtime: {
      external: 'EXTERNAL RUNTIME · Claude Code CLI · via ACP',
      suzent: 'SUZENT RUNTIME',
      bypassed: 'Suzent native model bypassed',
    },
  },
  fileChanges: {
    editedFiles: 'Edited {count} file(s)',
    diff: 'Diff',
    showMore: 'Show {count} more files',
    showLess: 'Show fewer files',
    openFile: 'Open file preview',
    undo: 'Undo',
    undone: 'Undone',
    undoing: 'Undoing…',
    undoSuccess: 'Restored {count} file(s).',
    undoNoChanges: 'Files were already restored.',
    conflict: 'Undo cancelled to protect manual changes in: {files}',
    binaryDiff: 'Binary change; text diff unavailable.',
  },
  conversationFork: {
    button: 'Branch from here',
    title: 'Branch conversation?',
    message:
      'A new independent conversation will start from this message. The original conversation and your current workspace files will stay unchanged.',
    cancel: 'Cancel',
    confirm: 'Create branch',
    creating: 'Creating…',
    errorTitle: 'Could not branch conversation',
    continuedFrom: 'Continued from task',
  },
  chatList: {
    pinError: 'Unable to update pin',
    pinErrorMessage: 'Please try again.',
    pinChat: 'Pin chat',
    unpinChat: 'Unpin chat',
    pinnedChats: 'Pinned',
    searchChatsPlaceholder: 'Search chats...',
    clearSearch: 'Clear search',
    newChat: 'New chat',
    newProject: 'New project',
    newProjectPlaceholder: 'Project name...',
    moveToProject: 'Move to project',
    currentLabel: 'Current',
    filter: {
      in: 'In',
      all: 'All projects',
      recentAll: 'Recent across all projects',
      recentIn: 'Recent in {name}',
    },
    sections: {
      automation: 'Automation',
      tasks: 'Tasks',
      chats: 'Chats',
      projects: 'Projects',
      searchResults: 'Search results',
    },
    automation: {
      scheduled: 'Scheduled tasks',
      heartbeat: 'Heartbeat',
      scheduledSummary: '{count} active · next {time}',
      activeCount: '{count} active',
      failedSummary: '{count} failed · needs attention',
      heartbeatSummary: '{count} sessions · checked {time}',
      heartbeatAttention: 'A heartbeat needs attention',
      healthy: 'Healthy',
      needsAttention: 'Attention',
      failed: 'Failed',
      nextRun: 'Next {time}',
      nextRunAt: 'Next run {time}',
      lastRunFailed: 'Last run failed · {error}',
      active: 'Active',
      disabled: 'Disabled',
      paused: 'Paused',
      everyMinutes: 'Every {minutes} min',
      notYet: 'Not yet',
      viewAllScheduled: 'View all {count} scheduled tasks',
      moreHeartbeat: '{count} more heartbeat sessions',
      deleteTaskTitle: 'Delete {name}?',
      deleteBoundTaskMessage: 'This removes the scheduled task. Its conversation will be kept.',
      deleteTaskMessage:
        'This removes the scheduled task and its conversation. This cannot be undone.',
    },
    organization: {
      title: 'Organize',
      byProject: 'By project',
      singleList: 'Single list',
    },
    kind: {
      you: 'You',
      subagents: 'Subagents',
      scheduled: 'Scheduled',
      all: 'All',
    },
    labels: {
      heartbeat: 'Heartbeat',
      running: 'Running',
      unreadCount: '{count} unread',
      subagentOf: 'Under {name}',
      subagentsCount: '{count} subagents',
      branchOf: 'Branched from {name}',
    },
    menu: {
      title: 'Actions',
      rename: 'Rename',
      moveToProject: 'Move to project',
      delete: 'Delete',
      deleteFailed: 'Could not delete project',
      deleteConfirmTitle: 'Delete {name}?',
      deleteConfirmEmpty: 'This project has no chats. This cannot be undone.',
      deleteConfirmWithChats:
        'This project has {count} chat(s). They will be moved to {target}, then the project will be deleted.',
    },
    empty: {
      noResultsTitle: 'No results found',
      noResultsDesc: 'Try a different search term',
      noChatsTitle: 'No chats yet',
      noChatsDesc: 'Start a new conversation to get started',
    },
    delete: {
      confirmTitle: 'Delete this chat?',
      confirm: 'Delete',
      deleting: 'Deleting...',
      buttonTitle: 'Delete chat',
      confirmMessage: 'This will permanently delete the chat and its messages.',
      cascadeMessage: 'This chat has sub-agents. Do you want to delete them too?',
      cascadeYes: 'Delete with sub-agents',
      cascadeNo: 'Delete chat only',
    },
    rename: {
      buttonTitle: 'Rename chat',
      placeholder: 'Chat name...',
      confirm: 'OK',
      cancel: 'Cancel',
    },
    untitled: 'Untitled chat',
    messagesCount: '{count} MSG',
    loadMore: 'Load more ({count} remaining)',
    loadingMore: 'Loading...',
  },
  chatInput: {
    engineGroups: {
      models: 'Models',
      acp: 'ACP agents',
    },
    acpNotInstalled: 'Not installed',
    acpInstallHint: 'Not installed — add it in Settings › ACP Agents',
    removeFile: 'Remove file',
    placeholderReady: 'How can I help you today?',
    placeholderLoading: 'System loading...',
    attachFiles: 'Attach files (images, PDFs, documents, etc.)',
    modelPlaceholder: 'Model',
    modelUpdateError: 'Could not update the model',
    thinkingTitle: 'Thinking',
    thinkingSliderLabel: 'Thinking effort',
    thinkingAutoLabel: 'Use the default thinking effort',
    thinkingManualLabel: 'Restore the previous manual thinking effort',
    thinkingModes: {
      auto: 'Auto',
      manual: 'Manual',
      off: 'Off',
    },
    thinkingValues: {
      auto: 'Default',
      off: 'Off',
      low: 'Low',
      medium: 'Med',
      high: 'High',
      xhigh: 'X-High',
    },
    thinkingDescriptions: {
      auto: "Keep the model's own reasoning default.",
      off: 'Answer without an extended reasoning pass.',
      low: 'Spend a small reasoning budget before answering.',
      medium: 'Spend a moderate reasoning budget before answering.',
      high: 'Spend a large reasoning budget before answering.',
      xhigh: 'Maximum reasoning budget — slowest, most deliberate answers.',
    },
    stopGenerating: 'Stop generating',
    sendMessage: 'Send message',
    redirectAgent: 'Redirect the agent',
    placeholderRedirect: 'Type to redirect the agent...',
    fileMentions: 'Files',
    fileMentionTypeToSearch: 'Type to search mounted files',
    fileMentionSearching: 'Searching files...',
    fileMentionNoResults: 'No matching files',
    stop: 'Stop',
    stopping: 'Stopping',
    send: 'Send',
    redirect: 'Redirect',
    runtime: {
      external: 'EXTERNAL RUNTIME',
      suzent: 'SUZENT RUNTIME',
      viaAcp: 'VIA ACP',
      bypassed: 'SUZENT NATIVE MODEL BYPASSED',
    },
    modelManagedByAcp: 'ACP MANAGED',
  },
  acp: {
    permission: {
      header: 'External agent needs approval',
      untitled: 'Untitled tool call',
      approved: 'Approved',
      denied: 'Denied',
      expired: 'No longer waiting for a decision',
    },
    sessionReset: {
      header: 'New session started',
      body: 'The previous {agent} session could not be resumed, so this turn starts fresh without earlier history.',
    },
  },
  chatWindow: {
    dragDropTitle: 'Drop files here',
    dragDropDesc: 'Images, PDFs, documents, etc.',
    scrollToBottom: 'Scroll to bottom',
    minimapLabel: 'Chat minimap',
    jumpToMessage: 'Jump to message {count}',
    minimapPreview: {
      user: 'You',
      assistant: 'Suzent',
      notice: 'Notice',
      activity: 'Activity',
      files: '{count} files',
      images: '{count} images',
    },
    connecting: 'Connecting to Neural Core...',
    outputValidationRetryError:
      'The model response format failed validation too many times. Please retry this message.',
    streamRecoveryUnavailable:
      'Stream recovery is no longer available. Reload to read the saved conversation.',
    streamPersistenceFailed:
      'The response could not be saved. Your received content is shown below.',
    streamInterrupted:
      'The connection was interrupted. Reload to reconnect; your received content is shown below.',
    genericError: 'Something went wrong. Please try again.',
    acpAgentLocked: 'ACP agent is fixed for this chat',
    heartbeatOff: 'Heartbeat off',
    permissionModeTitle: 'Tool permission mode',
    permissionModeLabel: 'Mode',
    autoModeConfirmation:
      'Smart mode uses a security classifier to approve low-risk actions without asking. Hard denies still apply. Enable Smart mode?',
    autoModeDialogTitle: 'Enable Smart mode?',
    autoModeCancel: 'Cancel',
    autoModeEnable: 'Enable',
    fullAccessModeConfirmation:
      'Full Access runs tool actions without asking for approval. Explicit deny rules and hard safety checks still apply. Enable Full Access?',
    fullAccessModeDialogTitle: 'Enable Full Access?',
    fullAccessModeEnable: 'Enable',
    permissionModes: {
      default: 'Ask',
      auto: 'Smart',
      full_access: 'Full Access',
    },
    permissionModeInputLabels: {
      default: 'Ask permissions',
      auto: 'Smart mode',
      full_access: 'Full Access',
    },
    permissionModeDescriptions: {
      default: 'Ask before state-changing tool actions.',
      auto: 'Classify unresolved actions automatically; hard safety rules still apply.',
      full_access: 'Run tool actions without approval prompts; explicit denies still apply.',
    },
    heartbeatRunning: 'Heartbeat running',
    heartbeatHealthy: 'Heartbeat healthy',
    heartbeatEnabled: 'Heartbeat enabled',
    heartbeatLastRun: 'Last heartbeat {time}',
    heartbeatLabel: 'Heartbeat',
    heartbeatConfigure: 'Configure heartbeat',
    heartbeatInterval: 'Run every',
    heartbeatMinutes: '{minutes}m',
    heartbeatHours: '{hours}h',
    heartbeatMinutesUnit: 'minutes',
    heartbeatInstructions: 'Instructions',
    heartbeatInstructionsPlaceholder: 'What should the agent do on each heartbeat?',
    hideToolCalls: 'Collapse Tools',
    showToolCalls: 'Expand Tools',
    sendConflict: 'Chat is still responding — reconnecting to the live response',
    sendFailed: 'Send failed ({status})',
    steerFailed: 'Steer failed ({status})',
    retryFailed: 'Retry failed ({status})',
    editFailed: 'Edit failed ({status})',
  },
  newChat: {
    creatingIn: 'Creating in',
    acp: {
      native: 'Native',
      agent: 'ACP agent',
      resume: 'Resume session',
      resuming: 'Resuming...',
      bound: 'bound',
      noAgents: 'No ACP agents',
      createFailed: 'Could not create ACP session',
      resumeFailed: 'Could not resume ACP session',
    },
    greetings: {
      nightOwl: 'Night owl?',
      goodMorning: 'Good morning.',
      keepBuilding: 'Keep building.',
      goodEvening: 'Good evening.',
      bedTime: 'Bed time? Or maybe late night coding?',
    },
  },
  chatMessage: {
    runtime: {
      external: 'EXTERNAL RUNTIME · {agent} · VIA ACP',
      suzent: 'SUZENT RUNTIME · {model}',
    },
    viewFile: 'View file (Shift+Click for full screen)',
    downloadFile: 'Download file',
    userLabel: 'User',
    modelSignature: 'Model: {model}',
    edit: 'Edit',
    rerun: 'Rerun',
    copy: 'Copy',
    resend: 'Resend',
    cancel: 'Cancel',
    editHint: 'Editing this message will resend it and remove the responses below.',
  },
  fileViewer: {
    title: 'File Preview',
    revealInExplorer: 'Reveal in Explorer',
    closeEsc: 'Close (Esc)',
    fetchFailed: 'Failed to fetch file content',
    errorPrefix: 'ERROR: ',
  },
  goal: {
    title: 'Active Goal',
    noGoal: 'No active goal.',
    turns: '{elapsed}/{max} turns',
    subgoals: 'Subgoals',
    pause: 'Pause',
    resume: 'Resume',
    clear: 'Clear',
    status: {
      active: 'ACTIVE',
      paused: 'PAUSED',
      completed: 'COMPLETED',
      cancelled: 'CANCELLED',
    },
  },
  task: {
    title: 'Tasks',
    noTasks: 'No tasks.',
    noTasksForChat: 'No tasks assigned to this chat.',
    assignee: '{name}',
    blocks: 'blocks {ids}',
    blockedBy: 'blocked by {ids}',
    status: {
      pending: 'TODO',
      inprogress: 'ACTIVE',
      completed: 'DONE',
      blocked: 'BLOCKED',
      cancelled: 'CANCELLED',
    },
  },
  projectBoard: {
    title: 'Project Board',
    noTasks: 'No tasks in this project.',
    noGoals: 'No active goals.',
    taskCount: 'tasks in project',
    open: 'Open Board',
  },
  archival: {
    title: 'Archival memory',
    filters: 'Filters',
    searchPlaceholder: 'Search memories... (try adding keywords)',
    searching: 'Searching...',
    sortBy: 'Sort by',
    sort: {
      newestFirst: 'Newest first',
      oldestFirst: 'Oldest first',
      highToLow: 'High → Low',
      lowToHigh: 'Low → High',
      mostAccessed: 'Most accessed',
      mostRelevant: 'Most relevant',
      relevanceNeedsQuery: 'Available while a search is active',
    },
    importanceLevel: 'Importance level',
    importance: {
      all: 'All',
      high: 'High',
      medium: 'Medium',
      low: 'Low',
      highRange: 'High (≥0.8)',
      mediumRange: 'Medium (0.5-0.8)',
      lowRange: 'Low (<0.5)',
    },
    source: {
      facts: 'Facts',
      notebook: 'Notebook',
      profile: 'Profile',
      all: 'Everything',
    },
    categoryLabel: 'Kind',
    category: {
      personal: 'About you',
      preference: 'Preference',
      project: 'Project',
      knowledge: 'Knowledge',
      concept: 'Concept',
      synthesis: 'Synthesis',
      entity: 'Person or place',
      literature: 'Reading',
      documentation: 'Documentation',
      inbox: 'Inbox',
      archive: 'Archive',
      asset: 'Asset',
      profile: 'Profile',
      uncategorized: 'Uncategorized',
    },
    tagLabel: 'Tags',
    clearFilters: 'Clear filters',
    clearAllFilters: 'Clear all filters',
    view: {
      list: 'List',
      cards: 'Cards',
      switchToCards: 'Switch to card view',
      switchToList: 'Switch to compact view',
    },
    showingCount: 'Showing {count} memories',
    group: {
      unknownDate: 'Undated',
    },
    showingOfTotal: 'Showing {count} of {total} memories',
    loadedOnlyNote: 'Sorting and filters apply to loaded memories only',
    filteredBy: 'filtered by {importance} importance',
    errorTitle: 'Error loading memories',
    empty: {
      noMatchesTitle: 'No matches found',
      noMatchesDesc: 'Try different search terms or adjust your filters',
      noMemoriesTitle: 'No memories yet',
      noMemoriesDesc: 'Memories will be saved here automatically as conversations happen',
      noImportanceDesc: 'No "{importance}" importance memories found. Try a different filter.',
    },
    loading: 'Loading memories...',
    loadMore: 'Load more',
  },
  memoryCard: {
    today: 'Today',
    yesterday: 'Yesterday',
    daysAgo: '{count} days ago',
    importance: {
      highShort: 'HIGH',
      medShort: 'MED',
      lowShort: 'LOW',
      tooltip: 'Importance: {value}',
    },
    delete: {
      confirmTitleCompact: 'Delete?',
      confirmYes: 'Yes',
      confirmTitle: 'Delete this memory?',
      confirmDelete: 'Yes, delete it',
      cancel: 'Cancel',
      button: 'Delete',
      buttonTitle: 'Delete memory',
    },
    badges: {
      new: 'NEW',
      hot: 'HOT',
    },
    showMore: 'Show more',
    showLess: 'Show less',
    meta: {
      created: 'CREATED:',
      views: 'VIEWS:',
      match: 'MATCH:',
      filterByTag: 'Filter by #{tag}',
    },
  },
  status: {
    copiedToClipboard: 'Copied to clipboard',
    codeCopiedToClipboard: 'Code copied to clipboard',
    logCopiedToClipboard: 'Log copied to clipboard',
    copyToClipboard: 'Copy to clipboard',
    backgroundChecking: 'Service checking',
    backgroundOnline: 'Service online',
    backgroundStarting: 'Service starting',
    backgroundOffline: 'Service offline',
    backgroundDetails: 'Scheduler {scheduler} · heartbeat {heartbeat} · {channels} channels',
  },
  codeBlock: {
    copyText: 'Copy text',
    copyCode: 'Copy code',
    collapse: 'Collapse',
    expand: 'Expand',
    code: 'Code',
    lines: '{count} lines',
    source: 'Source',
    preview: 'Preview',
    viewSource: 'View source',
    viewPreview: 'View preview',
  },
  logBlock: {
    systemLog: 'System log',
    collapse: 'Collapse',
    expand: 'Expand',
    lines: '{count} lines',
    chars: '{count} chars',
  },
  toolSummary: {
    verbs: {
      read: 'Read',
      write: 'Write',
      edit: 'Edit',
      findFiles: 'Find files',
      searchCode: 'Search code',
      run: 'Run',
      start: 'Start',
      checkOutput: 'Check output',
      stopCommand: 'Stop command',
      browser: 'Browser',
      openPage: 'Open page',
      searchWeb: 'Search web',
      ask: 'Ask',
      setGoal: 'Set goal',
      finishGoal: 'Finish goal',
      goal: 'Goal',
      addTask: 'Add task',
      addTasks: 'Add {count} tasks',
      updateTask: 'Update task',
      listTasks: 'List tasks',
      show: 'Show',
      makeImage: 'Make an image',
      lookAtImage: 'Look at image',
      say: 'Say',
      sendMessage: 'Send message',
      listContacts: 'List contacts',
      useSkill: 'Use skill',
      searchMemory: 'Search memory',
      searchChats: 'Search past chats',
      delegate: 'Delegate',
      listAgents: 'List agents',
      checkAgent: 'Check agent',
      messageAgent: 'Message agent',
      stopAgent: 'Stop agent',
    },
    /** Same verbs while the call is still in flight. */
    verbsActive: {
      read: 'Reading',
      write: 'Writing',
      edit: 'Editing',
      findFiles: 'Looking for files',
      searchCode: 'Searching code',
      run: 'Running',
      start: 'Starting',
      checkOutput: 'Checking output',
      stopCommand: 'Stopping command',
      browser: 'Browsing',
      openPage: 'Opening page',
      searchWeb: 'Searching web',
      ask: 'Asking',
      setGoal: 'Setting goal',
      finishGoal: 'Finishing goal',
      goal: 'Goal',
      addTask: 'Adding task',
      addTasks: 'Adding {count} tasks',
      updateTask: 'Updating task',
      listTasks: 'Listing tasks',
      show: 'Showing',
      makeImage: 'Making an image',
      lookAtImage: 'Looking at image',
      say: 'Saying',
      sendMessage: 'Sending message',
      listContacts: 'Listing contacts',
      useSkill: 'Using skill',
      searchMemory: 'Searching memory',
      searchChats: 'Searching past chats',
      delegate: 'Delegating',
      listAgents: 'Listing agents',
      checkAgent: 'Checking agent',
      messageAgent: 'Messaging agent',
      stopAgent: 'Stopping agent',
    },
    /** Same verbs once the call is over, for reading a transcript back. */
    verbsPast: {
      read: 'Read',
      write: 'Wrote',
      edit: 'Edited',
      findFiles: 'Looked for files',
      searchCode: 'Searched code',
      run: 'Ran',
      start: 'Started',
      checkOutput: 'Checked output',
      stopCommand: 'Stopped command',
      browser: 'Browsed',
      openPage: 'Opened page',
      searchWeb: 'Searched web',
      ask: 'Asked',
      setGoal: 'Set goal',
      finishGoal: 'Finished goal',
      goal: 'Goal',
      addTask: 'Added task',
      addTasks: 'Added {count} tasks',
      updateTask: 'Updated task',
      listTasks: 'Listed tasks',
      show: 'Showed',
      makeImage: 'Made an image',
      lookAtImage: 'Looked at image',
      say: 'Said',
      sendMessage: 'Sent message',
      listContacts: 'Listed contacts',
      useSkill: 'Used skill',
      searchMemory: 'Searched memory',
      searchChats: 'Searched past chats',
      delegate: 'Delegated',
      listAgents: 'Listed agents',
      checkAgent: 'Checked agent',
      messageAgent: 'Messaged agent',
      stopAgent: 'Stopped agent',
    },
    /** Every tool with no phrasing of its own: one frame, three tenses. */
    /** A call that came back an error. The verb keeps its proposal form. */
    failed: 'Failed to {verb}',
    unknownTool: {
      imperative: 'Call {name}',
      active: 'Calling {name}',
      past: 'Called {name}',
    },
    inScope: '{value} in {scope}',
    onChannel: '{value} on {channel}',
    namedAgent: '{type} agent',
    anyAgent: 'sub-agent',
    repeats: {
      read: 'Read {count} files',
      write: 'Wrote {count} files',
      edit: 'Edited {count} files',
      run: 'Ran {count} commands',
      searchWeb: 'Ran {count} web searches',
      searchCode: 'Searched code {count} times',
      findFiles: 'Looked for files {count} times',
      openPage: 'Opened {count} pages',
      browser: 'Took {count} browser actions',
      generic: '{name} · {count} times',
    },
    /** Same streak, still in flight — the rail says what is happening now. */
    repeatsActive: {
      read: 'Reading {count} files',
      write: 'Writing {count} files',
      edit: 'Editing {count} files',
      run: 'Running {count} commands',
      searchWeb: 'Running {count} web searches',
      searchCode: 'Searching code {count} times',
      findFiles: 'Looking for files {count} times',
      openPage: 'Opening {count} pages',
      browser: 'Taking {count} browser actions',
      generic: '{name} · {count} times',
    },
  },
  toolCallBlock: {
    permissionRejectPlaceholder: 'Optional reason or safer direction',
    permissionContractUnavailable:
      'This approval is missing its server permission contract. Retry the request.',
    permissionDecisionTitle: 'Permission decision',
    permissionOutcome: 'Outcome',
    permissionEvaluator: 'Evaluator',
    permissionReviewer: 'Reviewer',
    permissionReview: 'Review',
    permissionConfidence: 'Confidence',
    permissionConfidenceBadge: '{value} confidence',
    permissionConfidenceSummary: 'Confidence: {value}',
    permissionConfidenceLevels: {
      low: 'Low',
      medium: 'Medium',
      high: 'High',
    },
    permissionRisk: 'Risk',
    permissionCategories: 'Categories',
    permissionReason: 'Reason',
    permissionResolution: 'User resolution',
    permissionNotReviewed: 'Not reviewed',
    permissionFullAccess: 'Full access · not reviewed',
    permissionUserAllowed: 'User allowed',
    permissionUserDenied: 'User denied',
    permissionReviewRequired: 'Review required',
    permissionDenied: 'Denied',
    permissionAutoAllowed: 'Auto allowed',
    permissionRuleAllowed: 'Rule allowed',
    permissionPolicyAllowed: 'Policy allowed',
    ruleScopeSession: 'this chat',
    ruleScopeGlobal: 'all chats',
    ruleExplanationCommand: '{action} creates a rule for {scope}: `{command}`',
    ruleExplanationTool: '{action} creates a rule for {scope} covering the {tool} tool.',
    ruleExplanationGeneric: '{action} creates a scoped rule for {scope}.',
    permissionActions: {
      allowOnce: 'Allow',
      allowSession: 'Always allow for session',
      allowGlobal: 'Always allow globally',
      allowCommandSession: 'Allow this command for session',
      allowCommandGlobal: 'Always allow this command',
      reject: 'Reject',
    },
    done: 'Done',
    arguments: 'Arguments',
    output: 'Output',
    noArguments: '(no arguments)',
    resultOne: '{count} result',
    resultMany: '{count} results',
    viaEngine: 'via {engine}',
  },
  permissionDock: {
    title: 'May I {action}?',
    useTool: 'use {tool}',
    localProject: 'Project (local)',
    deny: 'Deny',
    allowOnce: 'Allow once',
    allowSession: 'Allow for session',
    allowGlobal: 'Always allow',
    ctrlEnter: 'Ctrl+Enter',
    yes: 'Yes',
    yesAnd: 'Yes, and {action}',
    yesAllowCommandSession: 'Yes, allow {command} for this session',
    yesAllowCommandGlobal: 'Yes, always allow {command}',
    yesAllowPrefixSession: 'Yes, allow all {command} … for this session',
    yesAllowPrefixGlobal: 'Yes, always allow all {command} …',
    no: 'No',
    feedbackPlaceholder: 'No, and tell Suzent what to do differently',
    submit: 'Submit',
  },
  activityRail: {
    thinking: 'Thinking',
    thought: 'Thought',
  },
  codeStepBlock: {
    thought: 'Thought',
    code: 'Code',
    executionLogs: 'Execution Logs',
    result: 'Result',
  },
  sandbox: {
    selectChat: 'Select a chat',
    back: 'Back',
    maximize: 'Maximize (full screen)',
    openInExplorer: 'Open in Explorer',
    uploadFile: 'Upload file',
    workspace: 'Workspace',
    shared: 'Shared',
    mounts: 'Mounts',
    scanning: 'Scanning...',
    up: '.. / Up',
    emptyDirectory: 'Empty directory',
    itemsCount: '{count} items',
    errorPrefix: 'ERROR: ',
    contextMenu: {
      label: 'File actions',
      open: 'Open',
      toggleFolder: 'Expand / collapse',
      reveal: 'Reveal in file manager',
      copyPath: 'Copy path',
      delete: 'Delete',
      deleteFileConfirm: 'Delete “{name}”? This cannot be undone.',
      deleteDirectoryConfirm: 'Delete “{name}” and everything inside it? This cannot be undone.',
      revealFailed: 'Could not reveal this item in the file manager',
      copyFailed: 'Could not copy the path',
      deleteFailed: 'Could not delete this item',
    },
    status: {
      syncing: 'SYNCING',
      ready: 'READY',
    },
  },
  deleteOverlay: {
    title: 'Delete?',
    confirm: 'Yes',
    cancel: 'No',
    buttonTitle: 'Delete',
  },
  select: {
    placeholder: 'Select...',
  },
  multiSelect: {
    placeholder: 'Select tools...',
    emptyMessage: 'No options',
  },
  skills: {
    loading: 'Loading skills...',
    errorTitle: 'Error',
    retry: 'Retry',
    title: 'Skills library',
    subtitle: 'Available capabilities index',
    searchPlaceholder: 'Search skills...',
    enabledOnly: 'Enabled only',
    enabledCount: '{enabled} of {total} enabled',
    sourceCount: '{count} skills',
    sources: {
      official: 'Bundled',
      user: 'User',
      external: 'External',
      repository: 'Repository',
      working: 'Working directory',
      home: 'Home',
      custom: 'Custom',
    },
    noMatches: 'No skills match this filter',
    reload: 'Reload from disk',
    enableSkill: 'Enable skill',
    disableSkill: 'Disable skill',
    openSkill: 'Open skill',
    closeSkill: 'Close skill',
    description: 'Description',
    mainBody: 'Main body',
    sandboxPath: 'Sandbox path',
    emptyTitle: 'No skills found',
    emptyDesc: "Add user skills to '~/.suzent/skills' to see them here.",
  },
  memoryStats: {
    totalMemories: 'Total memories',
    memoryUtilization: 'Memory utilization',
    utilizedCount: '{active}/{total} memories used',
    activity7d: '7d activity',
    recentCount: '{recent}/{total} accessed recently',
    coldMemoryRatio: 'Unaccessed ratio',
    coldCount: '{cold}/{total} still unaccessed',
    accessDistribution: 'Access distribution',
    refresh: 'Refresh stats',
    emptyHint: 'No memories recorded yet.',
    distributionTooltipUnaccessed: 'Unaccessed: {count}',
    distributionTooltipLight: 'Light (1-2): {count}',
    distributionTooltipEngaged: 'Engaged (3+): {count}',
    unaccessedCount: 'Unaccessed: {count}',
    lightCount: 'Light (1-2): {count}',
    engagedCount: 'Engaged (3+): {count}',
  },
  coreMemory: {
    saveFailed: 'Save failed',
    failedToSave: 'Failed to save',
    copiedText: '✓ Copied',
    placeholder: 'Enter {title}...',
    noContent: 'Nothing written here yet.',
    addContent: 'Add content',
    editHint: '⌘↩ save · esc cancel',
    charactersCount: '{count} characters',
    wordsApprox: '{count} words (approx)',
    labels: {
      persona: { title: 'Persona', desc: 'Identity, role, and workflow principles — persona.md' },
      user: { title: 'User', desc: 'Preferences, tech stack, and communication habits — user.md' },
      facts: {
        title: 'Long-term Memory',
        desc: 'Condensed knowledge and key decisions — MEMORY.md',
      },
      context: {
        title: 'Context',
        desc: 'Shared project context',
      },
    },
  },
  planProgress: {
    noActivePlan: 'No active plan.',
    progress: 'Progress',
    taskProgress: 'Task progress {completed}/{total}',
    current: 'Current: {title}',
    active: 'ACTIVE',
  },
  errorBoundary: {
    systemFailure: 'System Failure',
    unexpected: 'An unexpected error occurred',
    reboot: 'Reboot System',
    details: 'Error details',
  },
  folderContext: {
    activeContexts: 'Active contexts',
    noFoldersMounted: 'No folders mounted',
    remove: 'Remove',
    recentFolders: 'Recent folders',
    noRecentFolders: 'No recent folders',
    allRecentFoldersMounted: 'All recent folders are already mounted',
    chooseDifferent: 'Choose a different folder...',
    context: 'Working Dir',
  },
  fileLink: {
    clickToView: 'Click to view {path} (Shift+Click for full screen)',
  },
  imageViewer: {
    previous: 'Previous image',
    next: 'Next image',
    close: 'Close',
    fullScreenAlt: 'Full screen preview',
  },
} as const;
