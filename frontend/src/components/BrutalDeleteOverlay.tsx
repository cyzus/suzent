...
export const BrutalDeleteOverlay: React.FC<BrutalDeleteOverlayProps> = ({
  ...
  return (
    <div className={`absolute inset-0 bg-brutal-red z-30 animate-brutal-pop`}>
      ...
      <div
        className={`relative h-full flex ${isVertical ? 'flex-col items-center justify-center gap-2 p-2' : 'items-center justify-between px-3'} z-10`}
      >
        <span
          className={`${isVertical ? 'text-sm' : 'text-xs'} font-extrabold text-white text-center animate-brutal-glitch`}
        >
          {effectiveTitle}
        </span>
