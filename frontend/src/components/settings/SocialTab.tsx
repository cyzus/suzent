        ...
        <div className="flex items-start justify-between gap-3">
          <div className="min-w-0">
            ...
            <p className="text-xs text-neutral-600 dark:text-neutral-400 mt-1">
              {hasToken
                ? t('settings.social.wechatConnected')
                : t('settings.social.wechatAuthDesc')}
            </p>
          </div>
          <button
          ...
          >
            {wechatLoginBusy
              ? t('settings.social.wechatStarting')
              : t('settings.social.wechatLogin')}
          </button>
        </div>
        ...
        {wechatLogin && (
          <div className="flex flex-col sm:flex-row gap-3 items-start">
            ...
            <div className="space-y-1 text-xs font-mono text-neutral-600 dark:text-neutral-400">
              ...
              <div>
                {t('settings.social.wechatStatus')}: {wechatLogin.status}
              </div>
            </div>
          </div>
        )}
              ...
              <div className="space-y-3">
                ...
                {useCustomMcp && (
                  <div className="space-y-2 border-2 border-brutal-black p-3 bg-neutral-50 dark:bg-zinc-900">
                    {[...Object.keys(mcpServers.urls), ...Object.keys(mcpServers.stdio)].map(
                      (name) => (
                        <div key={name} className="flex items-center justify-between gap-3">
                          ...
                          <BrutalOnOff
                            size="sm"
                            checked={
                              socialConfig.mcp_enabled?.[name] ?? mcpServers.enabled[name] ?? true
                            }
                            onChange={(checked) =>
                              onConfigChange({
                                ...socialConfig,
                                mcp_enabled: {
                                  ...(socialConfig.mcp_enabled || {}),
                                  [name]: checked,
                                },
                              })
                            }
      ...
      <SettingsCard>
        ...
        {!handshakeEnabled && (
          <div className="border-2 border-dashed border-neutral-300 p-4 text-center dark:border-neutral-600">
            <p className="text-sm text-neutral-500 dark:text-neutral-400 font-mono">
              {t('settings.social.pairingDisabled')}
            </p>
          </div>
        )}
        ...
        {handshakeEnabled &&
          (pairings.length === 0 ? (
            <div className="border-2 border-dashed border-neutral-300 p-4 text-center dark:border-neutral-600">
              <p className="text-sm text-neutral-500 dark:text-neutral-400 font-mono">
                {t('settings.social.noPendingRequests')}
              </p>
            </div>
          ) : (
            <div className="space-y-3">
                ...
                >
                  <div className="min-w-0">
                    <div className="flex items-center gap-2 flex-wrap">
                      <span className="font-mono text-base font-black tracking-widest bg-brutal-yellow text-brutal-black border border-brutal-black px-2 py-0.5">
                        {p.token}
                      </span>
                    ...
                    {p.intro && (
                      <p className="text-sm text-neutral-700 dark:text-neutral-300 mt-1 truncate">
                        {p.intro}
                      </p>
                    )}
                  </div>
