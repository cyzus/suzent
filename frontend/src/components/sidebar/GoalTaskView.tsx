...
function TaskCheck({ status }: { status: TaskStatus }) {
  ...
  return (
    <div className="w-4 h-4 shrink-0 border-2 border-brutal-black bg-white dark:bg-zinc-700" />
  );
}

export const GoalTaskView: React.FC<GoalTaskViewProps> = ({
  goal,
  tasks,
  onOpenBoard,
  projectTaskCount,
  ...
  const runGoalAction = React.useCallback(
        ...
        {goal ? (
          <div className="p-3 space-y-2">
            <div className="flex items-center justify-between">
              ...
              <span
                className={`text-[8px] font-black px-1.5 py-0.5 border-2 ${GOAL_BADGE[goal.status]}`}
              >
              ...
              <div>
                <div className="flex justify-between text-[9px] font-black font-mono uppercase text-brutal-black dark:text-white mb-1">
                  ...
                  {goal.maxTurns != null && (
                    <span>
                      {t('goal.turns', {
                        elapsed: String(goal.turnsElapsed),
                        max: String(goal.maxTurns),
                      })}
                    </span>
                  )}
                </div>
                ...
                {subgoalsExpanded && (
                  <ul className="mt-1 space-y-0.5 pl-1 border-l-2 border-brutal-black ml-1">
                    {goal.subgoals.map((sg, i) => (
                      <li
                        key={i}
                        className="text-[10px] font-bold text-brutal-black dark:text-white pl-2"
                      >
                        · {sg}
                      </li>
                    ))}
                ...
                <div key={task.id} className={`border-2 p-2 ${TASK_BORDER[task.status]}`}>
                  <div className="flex items-start gap-2">
                    ...
                    <div className="flex-1 min-w-0">
                      <div
                        className={`text-xs font-black leading-snug text-brutal-black dark:text-white ${task.status === 'completed' ? 'line-through opacity-50' : ''}`}
                      >
                      ...
                      <div className="flex flex-wrap gap-x-2 mt-0.5">
                        ...
                        {task.blockedBy.length > 0 && (
                          <span className="text-[8px] font-black font-mono text-brutal-black bg-brutal-yellow px-1 border border-brutal-black">
                            {t('task.blockedBy', {
                              ids: task.blockedBy.map((b) => `#${b}`).join(', '),
                            })}
                          </span>
                        )}
