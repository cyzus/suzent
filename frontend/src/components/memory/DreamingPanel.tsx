...
function StatCard({
  ...
  return (
    <div className="border-2 border-brutal-black bg-neutral-50 dark:bg-zinc-900 overflow-hidden">
      ...
      <div className="p-3">
        <div className="text-[10px] font-bold uppercase tracking-wide text-neutral-500 dark:text-neutral-400 leading-tight">
          {label}
        </div>
        <div className="font-mono text-sm font-bold mt-1.5 truncate" title={value}>
          {value}
        </div>
      </div>
    </div>
...
function LastResultBox({
  ...
  const label = (() => {
    ...
    if (result.advanced)
      return t('settings.memoryConfig.lastResultAdvanced', { watermark: result.watermark || '—' });
    ...
    <div className="border-2 border-brutal-black bg-neutral-50 dark:bg-zinc-900 min-w-0 h-full flex flex-col">
      <div className="px-3 py-3 border-b-2 border-brutal-black">
        <div className="flex items-start justify-between gap-3">
          <div className="min-w-0">
            <div className="text-sm font-black uppercase tracking-wide leading-tight truncate">
              {title}
            </div>
          </div>
          <BrutalButton
      ...
      <div className="flex items-center justify-between gap-3 px-3 py-2 border-b-2 border-dashed border-neutral-300 dark:border-zinc-700">
        <span className="text-[10px] font-bold uppercase tracking-wide text-neutral-500 dark:text-neutral-400">
          {t('settings.memoryConfig.lastResult')}
        </span>
        <span className="font-mono text-[10px] font-bold text-neutral-500 dark:text-neutral-400 shrink-0">
      ...
      <div className="p-3 min-h-0 flex-1">
        {summary ? (
          <div
            className="h-44 overflow-y-auto overflow-x-hidden pr-1 text-xs leading-5 break-words"
            title={summary}
          >
...
export function DreamingPanel(): React.ReactElement {
  ...
  const statusLabel = (() => {
    ...
    if (dreamStatus.phase === 'finalizing')
      return t('settings.memoryConfig.dreamStatus.finalizing');
    if (dreamStatus.phase === 'queued' || dreamStatus.phase === 'preparing')
      return t('settings.memoryConfig.dreamStatus.preparing');
    if (dreamStatus.running || runningIngest || runningLint)
      return t('settings.memoryConfig.dreamStatus.running');
  ...
  const statusClass = (() => {
    if (dreamError || dreamStatus?.last_result?.advanced === false)
    ...
    <div className="border-3 border-brutal-black bg-white dark:bg-zinc-800 shadow-[4px_4px_0px_0px_rgba(0,0,0,1)] p-5">
          ...
          <div className="w-10 h-10 flex-shrink-0 grid place-items-center border-2 border-brutal-black bg-brutal-blue text-white">
            ...
            >
              <path
                ...
                d="M20 12.5A8.5 8.5 0 1111.5 4 6.5 6.5 0 0020 12.5z"
              />
            </svg>
          </div>
          <div className="min-w-0">
            <h3 className="font-brutal text-xl uppercase tracking-tight leading-tight">
              {t('settings.memoryConfig.dreamTitle')}
            </h3>
            <p className="text-xs text-neutral-600 dark:text-neutral-400 mt-0.5">
              {t('settings.memoryConfig.dreamDesc')}
            </p>
          </div>
        </div>
        <div className="flex items-center gap-2 shrink-0">
          <div
            className={`px-3 py-2 border-2 border-brutal-black font-mono text-xs font-bold uppercase whitespace-nowrap shadow-brutal-sm ${statusClass}`}
          >
            {isBusy && (
              <span className="inline-block w-1.5 h-1.5 rounded-full bg-current mr-1.5 align-middle animate-pulse" />
            )}
            {statusLabel}
          </div>
          <button
      ...
      <div className="mb-5">
        <div className="flex items-baseline justify-between gap-3 mb-2">
          ...
          <div className="font-mono text-xs font-bold tabular-nums">
            <span className={progressPercent >= 100 ? 'text-brutal-green' : ''}>
              {progressPercent}%
            </span>
            <span className="text-neutral-400 dark:text-neutral-500">
              {' '}
              ·{' '}
              {t('settings.memoryConfig.progressCount', {
                current: String(consolidatedCount),
                total: String(archiveCount),
              })}
            </span>
          </div>
        </div>
      ...
      <div className="grid grid-cols-1 xl:grid-cols-2 gap-4 items-stretch">
        ...
        >
          <div className="grid grid-cols-2 gap-2 sm:grid-cols-4">
            <StatCard
              label={t('settings.memoryConfig.consolidatedThrough')}
              value={dreamStatus?.watermark || '—'}
              accent="green"
            />
            <StatCard
              label={t('settings.memoryConfig.pendingLogs')}
              value={String(pendingCount)}
              accent={pendingCount > 0 ? 'amber' : 'neutral'}
            />
            <StatCard
              label={t('settings.memoryConfig.pendingFacts')}
              value={String(dreamStatus?.pending_facts ?? 0)}
              accent={(dreamStatus?.pending_facts ?? 0) > 0 ? 'amber' : 'neutral'}
            />
            <StatCard
              label={t('settings.memoryConfig.pendingConfirmations')}
              value={String(dreamStatus?.pending_confirmations ?? 0)}
              accent={(dreamStatus?.pending_confirmations ?? 0) > 0 ? 'amber' : 'neutral'}
            />
          </div>
        </LastResultBox>
        <LastResultBox
        ...
        >
          <div className="grid grid-cols-3 gap-2">
            ...
            <StatCard
              ...
              value={
                dreamStatus?.lint_due
                  ? t('settings.memoryConfig.lintDue')
                  : t('settings.memoryConfig.lintScheduled')
              }
            ...
            <StatCard
              ...
              value={
                dreamStatus?.lint_min_days
                  ? t('settings.memoryConfig.lintEveryDays', {
                      days: String(dreamStatus.lint_min_days),
                    })
                  : '—'
              }
              accent="neutral"
      ...
      {pendingDates.length > 0 && (
        <div className="font-mono text-[11px] text-neutral-500 dark:text-neutral-400 mt-3 pt-2 border-t border-dashed border-neutral-300 dark:border-zinc-700 truncate">
          {t('settings.memoryConfig.nextBatch')}:{' '}
          {pendingDates.slice(0, dreamStatus?.max_days ?? 14).join(', ')}
        </div>
      )}
      {dreamError && <div className="font-mono text-[11px] text-brutal-red mt-2">{dreamError}</div>}
    </div>
  );
}
