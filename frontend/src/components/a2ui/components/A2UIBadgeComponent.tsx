...
const COLOR_CLASSES: Record<string, string> = {
  default:
    'bg-neutral-100 border-neutral-400 text-neutral-700 dark:bg-zinc-700 dark:text-neutral-200',
...
export const A2UIBadgeComponent: React.FC<Props> = ({ component }) => {
  ...
  return (
    <div className="flex">
      <span className={`border-2 px-2 py-0.5 text-xs font-bold ${cls}`}>{label}</span>
    </div>
  );
};
