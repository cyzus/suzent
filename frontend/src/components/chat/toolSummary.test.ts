...
describe('getToolSummary', () => {
  it('summarizes file tools by basename', () => {
    expect(line('read_file', { file_path: '/repo/src/app/ToolCallBlock.tsx' }))
      .toBe('Read ToolCallBlock.tsx');
    expect(line('write_file', { file_path: 'D:\\workspace\\suzent\\notes.md', content: 'x' }))
      .toBe('Write notes.md');
    expect(line('edit_file', { file_path: 'src/main.py', old_string: 'a', new_string: 'b' }))
      .toBe('Edit main.py');
  });
  ...
  it('avoids tool jargon for the search tools', () => {
    expect(line('grep_search', { pattern: 'getToolSummary', include: '*.tsx' }))
      .toBe('Search code “getToolSummary” in *.tsx');
    expect(line('grep_search', { pattern: 'getToolSummary' }))
  ...
  it('promotes the model-written description to the headline for shell calls', () => {
    expect(line('run_command', { content: 'npm run build', description: 'Build the app' }))
      .toBe('Build the app npm run build');
  ...
  it('marks multi-line shell commands as truncated', () => {
    expect(line('run_command', { content: 'cd frontend\nnpm run build' }))
      .toBe('Run cd frontend …');
  });

  it('leads with the job a sub-agent was given, not the tool name', () => {
    expect(line('agent', { description: 'Audit the diff', subagent_type: 'Explore' }))
      .toBe('Audit the diff Explore agent');
  ...
  it('reads structured arguments for task tools', () => {
    expect(line('create_tasks', { tasks: [{ title: 'Wire up pills' }, { title: 'Add tests' }] }))
      .toBe('Add 2 tasks Wire up pills');
    expect(line('update_task', { task_id: 'T-3', status: 'completed' }))
      .toBe('Update task T-3 → completed');
  });
