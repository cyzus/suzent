    ...
    <div className="flex flex-col bg-neutral-50 dark:bg-zinc-900">
      {capabilities.map((capability) => {
        ...
        return (
          <section
            key={capability.id}
            aria-label={t('config.toolCatalog.capabilityAria', { name: capabilityName })}
          >
            <button
              type="button"
            ...
            >
              <span
                className={`mt-0.5 flex h-3.5 w-3.5 flex-shrink-0 items-center justify-center border-2 border-brutal-black ${allSelected ? 'bg-brutal-black' : someSelected ? 'bg-brutal-black/40' : 'bg-white dark:bg-zinc-900'}`}
              >
                {allSelected && (
                  <svg
              ...
              <span className="min-w-0">
                <span className="block text-[10px] font-black uppercase tracking-widest text-neutral-600 dark:text-neutral-300">
                  {capabilityName}
                </span>
                {capabilityDescription && (
                  <span className="mt-0.5 block text-[10px] font-medium leading-snug text-neutral-500 dark:text-neutral-400">
                    {capabilityDescription}
                  </span>
                )}
              </span>
                  ...
                  >
                    ...
                    >
                      ...
                      >
                        ...
                        {aiActive && (
                          <span className="text-[7px] font-black leading-none text-white">AI</span>
                        )}
                      </span>
                      <span className="min-w-0 flex-1">
                        ...
                        {toolDescription && (
                          <span className="mt-1 block text-[10px] font-medium normal-case leading-snug opacity-75">
                            {toolDescription}
                          </span>
                        )}
                        {tool.runtimeName && (
                          <span className="mt-1 block font-mono text-[9px] normal-case opacity-50">
                            {tool.runtimeName}
                          </span>
                        )}
                      </span>
