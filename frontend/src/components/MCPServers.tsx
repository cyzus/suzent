...
export const MCPServers: React.FC = () => {
  ...
  useEffect(() => {
    ...
    if (!prev) {
      ...
      if (
        ...
        !prevKeys.every((k, i) => k === newKeys[i] && prev[k] === enabledUrlDict[k])
      ) {
        changed = true;
      }
    }
  ...
  const toggle = (i: number) =>
    setServers((prev) => prev.map((s, idx) => (idx === i ? { ...s, enabled: !s.enabled } : s)));
    ...
    <div className="space-y-3 text-xs">
      ...
      <div className="flex gap-2">
        ...
        <input
          ...
          placeholder={t('config.mcp.url')}
          className="flex-[2] bg-neutral-800 border border-neutral-700 rounded px-2 py-1"
        />
        <button onClick={add} className="bg-brand-600 px-2 rounded">
          {t('common.add')}
        </button>
      </div>
      <ul className="space-y-2">
        {servers.map((s, i) => (
          <li key={i} className="flex items-center gap-2 bg-neutral-800 rounded p-2">
            ...
            <button onClick={() => remove(i)} className="text-neutral-400 hover:text-red-400">
              ✕
            </button>
          </li>
        ))}
