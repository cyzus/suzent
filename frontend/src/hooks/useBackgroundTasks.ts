/**
 * useBackgroundTasks — subscribes to /background-tasks/stream (SSE) to track
 * background tasks in real-time. Used by StatusBar, ChatWindow and the sidebar.
 *
...
interface BackgroundTaskStatusState {
  activeTasks: BackgroundTaskSummary[];
  /** Latest known state of every task seen this session, terminal ones included. */
  taskStates: Record<string, BackgroundTaskSummary>;
...
let _activeTasks: BackgroundTaskSummary[] = [];
let _taskStates: Record<string, BackgroundTaskSummary> = {};
...
function _recordState(task: BackgroundTaskSummary): boolean {
...
function _upsertTask(task: BackgroundTaskSummary) {
...
function _applyTask(task: BackgroundTaskSummary) {
...
async function _pollUnresolved() {
  ...
  const results = await Promise.all(
    ids.map(async (id) => {
      try {
        const res = await fetch(`${getApiBase()}/background-tasks/${id}`);
        if (!res.ok) return null;
        const task = (await res.json())?.task as BackgroundTaskSummary | undefined;
  ...
  const seen = results.filter((task): task is BackgroundTaskSummary => task !== null);
...
function _openEventSource() {
  if (_es) return;
  _es = new EventSource(withAuthQuery(`${getApiBase()}/background-tasks/stream`));
  ...
  _es.onmessage = (evt) => {
    try {
      ...
      if (msg.event === 'snapshot') {
        _activeTasks = [];
        (msg.tasks ?? []).forEach(_applyTask);
        notify();
      } else if (msg.event === 'task_update') {
        _applyTask(msg.task as BackgroundTaskSummary);
        notify();
      }
    } catch {
      /* ignore parse errors */
...
export function useBackgroundTasks(): BackgroundTaskStatusState {
  ...
  const finish = (taskId: string, patch: Partial<BackgroundTaskSummary>) => {
    const known = _taskStates[taskId] ?? _activeTasks.find((t) => t.task_id === taskId);
    if (known) _recordState({ ...known, ...patch } as BackgroundTaskSummary);
