...
export function useSlashCommands(input: string) {
  ...
  useEffect(() => {
    fetchCommands()
      .then((cmds) =>
        setCommands(
          [...cmds].sort((a, b) => (a.category || 'tools').localeCompare(b.category || 'tools'))
        )
      )
      .catch(() => {});
  }, []);
  ...
  const suggestions = useMemo(() => {
    ...
    if (hasSpace) {
      ...
      const exactCmd = filtered.find((cmd) => cmd.aliases.some((a) => a.toLowerCase() === query));
      ...
      if (exactCmd && exactCmd.options && Object.keys(exactCmd.options).length > 0) {
        ...
        if (parts.length <= 2) {
          ...
          const optionKeys = Object.keys(exactCmd.options).filter((k) =>
            k.toLowerCase().startsWith(optionQuery)
          );
