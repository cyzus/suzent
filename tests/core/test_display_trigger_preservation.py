...
# --- the crash window between snapshot and background persist ---------------
...
class _FakeDB:
    def __init__(self, messages=None):
        self.chat = _FakeChat(list(messages or []))
        self.writes = 0

    def get_chat(self, chat_id):
        return self.chat

    def update_chat(self, chat_id, messages=None, **kwargs):
        self.writes += 1
        self.chat.messages = list(messages or [])


def test_trigger_row_is_written_before_the_run(monkeypatch):
    """The fast state snapshot can land and the process exit before background
    persistence, leaving the trigger durable in history but unrecorded. After a
    restart nothing can vouch for it, so the row has to exist up front."""
    from suzent.core import chat_processor as cp

    db = _FakeDB()
    monkeypatch.setattr(cp, "get_database", lambda: db)

    cp._prewrite_trigger_display_row("chat-1", "Cron: digest", TS)

    assert db.chat.messages == [
        {
...
def test_the_prewritten_row_is_what_restoration_matches(monkeypatch):
    """End to end: what the pre-write stores must satisfy the matcher."""
    from suzent.core import chat_processor as cp
    ...
    out = _preserve_display_triggers([_rebuilt("Cron: digest")], db.chat.messages)

    assert out[0]["role"] == "system_triggered"


def test_prewrite_does_not_duplicate_an_identical_row(monkeypatch):
    from suzent.core import chat_processor as cp
    ...
    assert len(db.chat.messages) == 1
    assert db.writes == 1


def test_prewrite_is_non_fatal_when_the_database_fails(monkeypatch):
    from suzent.core import chat_processor as cp

    def boom():
        raise RuntimeError("db down")

    monkeypatch.setattr(cp, "get_database", boom)

    cp._prewrite_trigger_display_row("chat-1", "Cron: digest", TS)  # must not raise


def test_prewrite_skips_turns_with_no_trigger(monkeypatch):
    from suzent.core import chat_processor as cp

    db = _FakeDB()
    monkeypatch.setattr(cp, "get_database", lambda: db)

    cp._prewrite_trigger_display_row("chat-1", "", TS)
