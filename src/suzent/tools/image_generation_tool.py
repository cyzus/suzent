...
class ImageGenerationTool(Tool):
    ...
    async def forward(self, ctx: RunContext[AgentDeps], prompt: str) -> str:
        ...
        try:
            ...
            if chat_id:
                workspace_dir = Path(CONFIG.sandbox_data_path) / "sessions" / chat_id / "images"
            else:
            ...
            async with aiohttp.ClientSession() as session:
                async with session.get(image_url) as response:
                    if response.status == 200:
                        ...
                        logger.error(f"Failed to download generated image. Status: {response.status}")
                        return f"Generated image at {image_url} but failed to download it."
