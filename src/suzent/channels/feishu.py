...
class FeishuChannel(SocialChannel):
    ...
    def __init__(self, config: Dict[str, Any]):
        ...
        if not lark:
            logger.error("lark-oapi not installed. Install with `pip install lark-oapi`.")
    ...
    async def connect(self):
        ...
        def do_p2_im_message_receive_v1(data: P2ImMessageReceiveV1) -> None:
            ...
            if self.main_loop:
                future = asyncio.run_coroutine_threadsafe(self._handle_event(data), self.main_loop)
    ...
    async def _handle_event(self, data: P2ImMessageReceiveV1):
        ...
        try:
            ...
            logger.debug(f"Feishu Message IDs - Union: {u_id}, Open: {o_id}, User: {us_id}")
            ...
            if msg.message_type == "text":
                ...
                local_path = await self._download_file(image_key, msg.message_id, "image", ".jpg")
                if local_path:
                    attachments.append({
                        "type": "image",
                        "path": local_path,
                ...
                local_path = await self._download_file(file_key, msg.message_id, "file", ext, filename=file_name)
                if local_path:
                    attachments.append({
                        "type": "file",
                        "path": local_path,
    ...
    async def _download_file(self, file_key: str, message_id: str, type_str: str, ext: str, filename: str = None) -> Optional[str]:
        ...
        try:
            ...
            def do_download():
                ...
                if not response.success():
                    logger.error(f"Feishu download failed for {file_key}: {response.code} {response.msg}")
                    return None
    ...
    async def send_message(self, target_id: str, content: str, **kwargs) -> bool:
        ...
        try:
            ...
            resp = await asyncio.to_thread(lambda: self.client.im.v1.message.create(req))
            
            if not resp.success():
                logger.error(f"Failed to send Feishu message: {resp.code} {resp.msg} - ReqID: {resp.request_id}")
                logger.error(f"Failed Body: {resp.raw.content if hasattr(resp, 'raw') else 'Unknown'}")
                return False
    ...
    async def send_file(self, target_id: str, file_path: str, caption: str = None, **kwargs) -> bool:
        ...
        await self.send_message(target_id, f"File sending not yet implemented for Feishu. (File: {Path(file_path).name})")
        return True
